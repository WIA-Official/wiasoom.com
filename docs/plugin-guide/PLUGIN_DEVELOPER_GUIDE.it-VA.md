...
- [Parte 2: Riferimento API del Contesto del Plugin](#parte-2-riferimento-api-del-contesto-del-plugin)
  ...
  - [ctx.settings](#ctxsettings--memorizzazione-persistente)
...
> **Stato: In Arrivo** — L'API AI è definita ma non ancora collegata a Soomy. Attualmente restituisce `{ response: 'AI non ancora collegata' }`. È prevista un'integrazione completa dell'AI per una futura versione.
...
| **MAI** recuperare URL esterni | Eccezione: endpoint API `wiasoom.com` |
...
Un plugin che gestisce un elenco TODO utilizzando impostazioni per la memorizzazione persistente e un webview per la visualizzazione.
