...
<h1 align="center">Guía de Desarrollador de Plugins de WIA SOOM</h1>
<p align="center"><strong>Construí tu propio plugin en 5 minutos.</strong></p>
<p align="center">Creá herramientas poderosas para servidores, paneles de control y automatizaciones — directamente dentro de WIA SOOM.</p>
...
- [Parte 2: Referencia de la API de Contexto del Plugin](#parte-2-referencia-de-la-api-de-contexto-del-plugin)
...
```
### Paso 4: Reiniciar WIA SOOM

Reiniciá la aplicación (o alterná el plugin apagado/encendido en Configuración → Plugins).

Deberías ver un botón **"Hola Mundo"** en la barra lateral. Hacé clic en él — ¡verás una notificación de éxito!
...
## Parte 2: Referencia de la API de Contexto del Plugin
...
Suscribirse a toda la salida de una sesión de terminal. Devuelve una **función de cancelación de suscripción**.

| Parámetro | Tipo | Descripción |
|-----------|------|-------------|
| `sessionId` | `string` | La sesión de terminal a observar |
| `callback` | `(data: string) => void` | Se llama con cada fragmento de salida |
| **Devuelve** | `() => void` | Llamá esto para dejar de escuchar |
```javascript
...
unsubscribe();
```
**Importante:** Siempre guardá la función de cancelación de suscripción y llamala en `deactivate()` para prevenir fugas de memoria.

---

### `ctx.sftp` — Transferencia de archivos

> **Estado: Próximamente** — La API de SFTP está definida pero aún no conectada al motor SFTP de la aplicación. `list()` actualmente devuelve un array vacío, y `upload()`/`download()` no hacen nada. Esto se implementará completamente en una futura versión. Por ahora, usá `ctx.terminal.send()` con comandos `scp` o `rsync` como solución alternativa.
...
| `label` | `string` | Sí | Texto del botón que se muestra en la barra lateral |
...
```
**Referencia de iconos:** Navegá todos los iconos disponibles en [lucide.dev/icons](https://lucide.dev/icons)

> **Nota de compatibilidad:** Algunos plugins más antiguos usan argumentos posicionales como `addSidebarButton(id, icon, label, onClick)`. La API oficial utiliza un **objeto de opciones** como se documenta arriba. Siempre usá el estilo de objeto para nuevos plugins.

#### `ui.openWebview(options)`

Abre una ventana emergente con contenido HTML personalizado. Así es como construís UIs ricas.

| Opción | Tipo | Descripción |
|--------|------|-------------|
| `title` | `string` | Título de la ventana |
| `html` | `string` | Contenido HTML completo para renderizar |
```javascript
context.ui.openWebview({
  title: 'Server Status',
  html: `
...
```
> Echa [Parte 3](#part-3-building-custom-ui-with-webviews) para patrones avanzados de webview.
...
> **Nota:** Los webviews son solo de visualización — no pueden llamar a las APIs del plugin. Usa `ctx.settings` en tus manejadores de botones de la barra lateral para modificar configuraciones, y usa `openWebview()` para mostrar el estado actual.
```javascript
...
Utiliza estas en tu `package.json` `keywords` o al enviar al registro:

| Categoría | Descripción |
|-----------|-------------|
| `server` | Administración general del servidor |
...
| `web` | Administración de servidores web |
