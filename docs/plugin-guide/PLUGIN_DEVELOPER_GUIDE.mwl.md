...
<h1 align="center">Guia de Desenvolvedor de Plugins WIA SOOM</h1>
<p align="center"><strong>Construa o seu próprio plugin em 5 minutos.</strong></p>
...
- [Parte 2: Referência da API do Contexto do Plugin](#parte-2-referência-da-api-do-contexto-do-plugin)
...
Reinicie o aplicativo (ou altere o plugin para desligado/ligado em Configurações → Plugins).
...
## Parte 2: Referência da API do Contexto do Plugin
...
|-----------|------|-------------|
| `sessionId` | `string` | A sessão de terminal para enviar |
| `data` | `string` | O comando ou dado a ser enviado |
§§§CHUNK_SEPARATOR§§§
#### `terminal.onOutput(sessionId, callback)`

Inscreva-se para toda a saída de uma sessão de terminal. Retorna uma **função de cancelamento**.

| Parâmetro | Tipo | Descrição |
|-----------|------|-------------|
| `sessionId` | `string` | A sessão de terminal a ser observada |
...
§§§CHUNK_SEPARATOR§§§
**Solução alternativa (até que a API SFTP esteja ativa):**
§§§CHUNK_SEPARATOR§§§
---
...
| `label` | `string` | Sim | Texto do botão mostrado na barra lateral |
...
> **Nota de compatibilidade:** Alguns plugins mais antigos usam argumentos posicionais como `addSidebarButton(id, icon, label, onClick)`. A API oficial usa um **objeto de opções** como documentado acima. Sempre use o estilo de objeto para novos plugins.
...
Mostra uma notificação em forma de toast.

| Parâmetro | Tipo | Descrição |
|-----------|------|-------------|
| `type` | `'success' \| 'error' \| 'info'` | Estilo da notificação |
| `message` | `string` | Texto a ser mostrado |
§§§CHUNK_SEPARATOR§§§
...
> **Status: Em Breve** — A API de IA está definida, mas ainda não está conectada ao Soomy. Atualmente retorna `{ response: 'AI not yet connected' }`. A integração completa de IA está planejada para um lançamento futuro.
...
### Padrão: Comando do Terminal → Analisar Saída → Mostrar em HTML
...
3. Verifique se funciona: botão da barra lateral aparece, recursos funcionam corretamente
...
```
**Campos obrigatórios de `package.json`:**
...
2. **Adicione** seu plugin a `plugins/{seu-nome-do-plugin}/`
...
Use estes nas suas `package.json` `keywords` ou ao submeter ao registro:

| Categoria | Descrição |
|----------|-------------|
| `server` | Gestão geral de servidores |
...
| `ai` | Ferramentas alimentadas por IA |
| `biotech` | Ferramentas de biotecnologia |
| `space` | Ferramentas de espaço e astronomia |
| `network` | Ferramentas de rede |
| `database` | Gestão de bases de dados |
| `monitoring` | Monitoramento de servidores |
...
<p align="center"><em>— A Equipa WIA SOOM</em></p>
```javascript
