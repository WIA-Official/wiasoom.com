...
- [第1部分：快速入门 — 你的第一个插件在5分钟内](#part-1-quick-start--your-first-plugin-in-5-minutes)
...
- [第4部分：发布你的插件](#part-4-publishing-your-plugin)
- [第5部分：最佳实践](#part-5-best-practices)
- [第6部分：实际案例](#part-6-real-world-examples)
- [附录：类别与图标](#appendix-categories--icons)

---

## 第1部分：快速入门 — 你的第一个插件在5分钟内

### 你将构建什么

一个“Hello World”插件，向侧边栏添加一个按钮。点击时，它会显示一个通知。
...
当你的 `activate(context)` 函数被调用时，`context`（或 `ctx`）提供以下 API：
§§§CHUNK_SEPARATOR§§§
---
...
> **状态：即将推出** — SFTP API 已定义，但尚未连接到应用程序的 SFTP 引擎。`list()` 当前返回一个空数组，而 `upload()`/`download()` 是无操作的。这将在未来的版本中完全实现。现在，请使用 `ctx.terminal.send()` 搭配 `scp` 或 `rsync` 命令作为替代方案。
...
> **兼容性说明：** 一些旧插件使用位置参数，如 `addSidebarButton(id, icon, label, onClick)`。官方 API 使用如上所述的 **选项对象**。新插件始终使用对象风格。
...
> **状态：即将推出** — AI API 已定义，但尚未连接到 Soomy。目前返回 `{ response: 'AI not yet connected' }`。完整的 AI 集成计划在未来的版���中实现。
...
> **重要限制：** Webviews 是 **仅用于显示**。它们不能回调插件 API（`ctx.settings`、`ctx.terminal` 等）。使用侧边栏按钮进行所有用户操作，并使用 `openWebview()` 显示当前状态。如果需要交互功能，请通过侧边栏按钮触发它们，并重新打开 webview 以刷新显示。
...
> **注意：** Webviews 仅用于显示 — 它们不能回调插件 API。在侧边栏按钮处理程序中使用 `ctx.settings` 来修改设置，并使用 `openWebview()` 显示当前状态。
```javascript
...
2. 重启 WIA SOOM
3. 验证它是否工作：侧边栏按钮出现，功能正常
4. 测试边缘情况：如果没有终端连接会发生什么？
...
- **安全性** — 没有危险的 API（见 [安全规则](#security-rules)）
- **质量** — 它能工作吗？代码干净吗？
- **实用性** — 它解决了实际问题吗？
...
3. 你的插件会出现在所有 WIA SOOM 用户的 **插件商���** 中！
...
一个使用设置进行持久存储和 webview 进行显示的 TODO 列表管理插件。

> **设计模式：** 由于 webview 不能直接调用插件 API，因此该插件使用“快照”方法 — 它从设置中读取 TODO，将其渲染为只读 HTML，并提供基于侧边栏的操作以添加项目。webview 是一个 **显示** 层，而不是交互式表单。
```javascript
...
在你的 `package.json` `keywords` 中使用这些类别，或者在提交到注册表时使用：
