...
Inscreva-se em toda a saída de uma sessão de terminal. Retorna uma **função de cancelamento**.
...
unsubscribe();
```
**Importante:** Sempre salve a função de cancelamento e a chame em `deactivate()` para evitar vazamentos de memória.
...
| `icon` | `string` | Sim | Nome do ícone Lucide (por exemplo, `'server'`, `'shield'`, `'database'`) |
...
> **Nota:** Webviews são apenas para exibição — elas não podem chamar de volta as APIs do plugin. Use `ctx.settings` nos manipuladores de botões da barra lateral para modificar configurações e use `openWebview()` para mostrar o estado atual.
```javascript
...
| Categoria | Descrição |
|----------|-------------|
| `server` | Gerenciamento geral de servidores |
...
| `web` | Gerenciamento de servidores web |
...
- **Problemas com Plugins:** [GitHub Issues](https://github.com/WIA-Official/wiasoom.com/issues)
