...
<h1 align="center">دليل مطور إضافات WIA SOOM</h1>
...
- [الجزء 1: البدء السريع — إضافتك الأولى في 5 دقائق](#part-1-quick-start--your-first-plugin-in-5-minutes)
...
- [ملحق: الفئات والرموز](#appendix-categories--icons)

---

## الجزء 1: البدء السريع — إضافتك الأولى في 5 دقائق
...
أعد تشغيل التطبيق (أو قم بتبديل الإضافة من التشغيل/الإيقاف في الإعدادات → الإضافات).

يجب أن ترى زر **"مرحبا بالعالم"** في الشريط الجانبي. انقر عليه — سترى إشعار ��جاح!
...
عند استدعاء دالة `activate(context)` الخاصة بك، يوفر `context` (أو `ctx`) هذه الواجهات:
§§§CHUNK_SEPARATOR§§§
---
...
الاشتراك في جميع المخرجات من جلسة الطرفية. يعيد **دالة إلغاء الاشتراك**.

| المعامل | النوع | الوصف |
|---------|------|-------|
| `sessionId` | `string` | جلسة الطرفية التي سيتم مراقبتها |
| `callback` | `(data: string) => void` | يتم استدعاؤها مع كل جزء من المخرجات |
| **يعيد** | `() => void` | استدعِ هذه لإيقاف الاستماع |
§§§CHUNK_SEPARATOR§§§
...
> **الحالة: قريبا** — تم تعريف واجهة برمجة تطبيقات SFTP ولكن لم يتم توصيلها بعد بمحرك SFTP الخاص بالتطبيق. حاليًا، تعيد `list()` مصفوفة فارغة، و`upload()`/`download()` لا تفعل شيئًا. سيتم تنفيذ ذلك بالكامل في إصدار مستقبلي. في الوقت الحالي، استخدم `ctx.terminal.send()` مع أوامر `scp` أو `rsync` كحل بديل.
...
§§§CHUNK_SEPARATOR§§§
**حل بديل (حتى تصبح واجهة برمجة تطبيقات SFTP نشط��):**
§§§CHUNK_SEPARATOR§§§
---
...
| `onClick` | `() => void` | نعم | الدالة المستدعاة عند النقر على الزر |
§§§CHUNK_SEPARATOR§§§
...
```
> انظر [الجزء 3](#part-3-building-custom-ui-with-webviews) للحصول على أنماط الويب المتقدمة.
...
تُخزن إعدادات الإضافة بشكل دائم في `~/.wia-soom/plugins/{your-plugin}/.plugin-settings.json`.
...
> **الحالة: قادم قريبًا** — تم تعريف واجهة برمجة تطبيقات الذكاء الاصطناعي ولكن لم يتم توصيلها بعد بـ Soomy. حاليًا ترجع `{ response: 'AI not yet connected' }`. من المخطط تنفيذ تكامل كامل للذكاء الاصطناعي في إصدار مستقبلي.
...
تتيح لك واجهة برمجة تطبيقات `openWebview()` بناء واجهات لوحات المعلومات باستخدام HTML وCSS وJavaScript — كل ذلك داخل نافذة منبثقة.

> **حدود مهمة:** Webviews هي **للعرض فقط**. لا يمكنها الاتصال مرة أخرى بواجهات برمجة تطبيقات الإضافات (`ctx.settings`, `ctx.terminal`, إلخ). استخدم أزرار الشريط الجانبي لجميع إجراءات المستخدم، واستخدم `openWebview()` لعرض الحالة الحالية. إذا كنت بحاجة إلى ميزات تفاعلية، قم بتفعيلها من أزرار الشريط الجانبي وأعد فتح Webview لتحديث العرض.
...
> **ملاحظة:** Webviews هي للعرض فقط — لا يمكنها الاتصال مرة أخرى بواجهات برمجة تطبيقات الإضافات. استخدم `ctx.settings` في معالجات أزرار الشريط الجانبي الخاصة بك لتعديل الإعدادات، واستخدم `openWebview()` لعرض الحالة الحالية.
```javascript
...
### الخطوة 1: الاختب��ر محليًا

1. انسخ الإضافة الخاصة بك إلى `~/.wia-soom/plugins/{your-plugin}/`
2. أعد تشغيل WIA SOOM
3. تحقق من أنها تعمل: ظهور زر الشريط الجانبي، وعمل الميزات بشكل صحيح
4. اختبار الحالات الحدودية: ماذا يحدث إذا لم يكن هناك طرفية متصلة؟

### الخطوة 2: التحضير للتقديم

يجب أن يحتوي مجلد الإضافة الخاصة بك على:
```javascript
...
| **لا تستخدم أبداً** الوصول إلى `process.env` | قد تحتوي المتغيرات البيئية على أسرار |
| **لا تستخدم أبداً** `require('fs')` مباشرة | استخدم `ctx.settings` للتخزين، و `ctx.sftp` لنقل ا��ملفات |
| **لا تستخدم أبداً** حزم npm الخارجية | JavaScript فقط — لا node_modules |
...
يشغل `df -h` على الخا��م البعيد ويظهر المساحة المستخدمة/المتاحة في شريط الحالة.
```javascript
...
إضافة تدير قائمة TODO باستخدام الإعدادات للتخزين الدائم وعرض عبر webview.

> **نمط التصميم:** بما أن webviews لا يمكنها استدعاء واجهات برمجة التطبيقات للإضافات مباشرة، تستخدم هذه الإضافة نهج "لقطة" — تقرأ TODOs من الإعدادات، تعرضها كـ HTML للقراءة فقط، وتوفر إجراءات قائمة جانبية لإضافة العناصر. الـ webview هو طبقة **عرض**، وليس نموذج تفاعلي.
```javascript
...
<p align="center"><em>اصنع شيئًا مذهلاً. شاركه مع العالم.</em></p>
