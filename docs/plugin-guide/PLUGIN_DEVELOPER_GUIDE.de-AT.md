...
Ein "Hello World" Plugin, das einen Button zur Seitenleiste hinzufügt. Wenn Sie darauf klicken, wird eine Benachrichtigung angezeigt.
...
> **Wichtige Einschränkung:** Webviews sind **nur zur Anzeige**. Sie können nicht auf Plugin-APIs (`ctx.settings`, `ctx.terminal` usw.) zurückgreifen. Verwenden Sie Sidebar-Buttons für alle Benutzeraktionen und `openWebview()`, um den aktuellen Zustand anzuzeigen. Wenn Sie interaktive Funktionen benötigen, lösen Sie diese über Sidebar-Buttons aus und öffnen Sie das Webview erneut, um die Anzeige zu aktualisieren.
...
| `description` | Einzeiliger Beschreibungstext | `"Überwacht nginx-Zugriffsprotokolle in Echtzeit"` |
...
2. **Fügen** Sie Ihr Plugin in `plugins/{your-plugin-name}/` hinzu
...
1. Ihr Plugin wird in `registry.json` hinzugefügt
2. Ein ZIP-Paket wird in `dist/` erstellt
...
| Regel | Warum |
|------|-----|
| **NIEMALS** `eval()` oder `new Function()` verwenden | Risiko von Code-Injection |
...
Ein Plugin, das eine TODO-Liste verwaltet, wobei Einstellungen für persistenten Speicher und eine Webansicht zur Anzeige verwendet werden.
