...
<p align="center">Maak krachtige servertools, dashboards en automatiseringen — direct binnen WIA SOOM.</p>

---

## Table of Contents

- [Part 1: Quick Start — Je Eerste Plugin in 5 Minuten](#part-1-quick-start--je-eerste-plugin-in-5-minuten)
- [Part 2: Plugin Context API Referentie](#part-2-plugin-context-api-referentie)
  - [ctx.terminal](#ctxterminal--voeren-commando's-uit-op-afstandsservers)
  - [ctx.sftp](#ctxsftp--bestandsoverdracht)
  - [ctx.ui](#ctxui--gebruikersinterface)
  - [ctx.settings](#ctxsettings--blijvende-opslag)
  - [ctx.ai](#ctxai--ai-integratie)
- [Part 3: Bouw Aangepaste UI met Webviews](#part-3-bouw-aangepaste-ui-met-webviews)
- [Part 4: Publiceren van Je Plugin](#part-4-publiceren-van-je-plugin)
- [Part 5: Beste Praktijken](#part-5-beste-praktijken)
- [Part 6: Voorbeelden uit de Praktijk](#part-6-voorbeelden-uit-de-praktijk)
- [Appendix: Categorieën & Iconen](#appendix-categorieën--iconen)

---

## Part 1: Quick Start — Je Eerste Plugin in 5 Minuten

### Wat je gaat bouwen

Een "Hello World" plugin die een knop aan de zijbalk toevoegt. Wanneer erop geklikt wordt, toont het een notificatie.
...
```
**Verplichte velden:** `name`, `version`, `description`, `author`, `main`
...
Je zou een **"Hello World"** knop in de zijbalk moeten zien. Klik erop — je zult een succesnotificatie zien!
...
## Part 2: Plugin Context API Referentie
...
### `ctx.terminal` — Voeren commando's uit op afstandservers
...
|-----------|------|-------------|
| `sessionId` | `string` | De terminalsessie om naar te sturen |
| `data` | `string` | Het commando of de data om te sturen |
```javascript
...
| Parameter | Type | Beschrijving |
|-----------|------|-------------|
| `sessionId` | `string` | De terminalsessie om te bekijken |
...
> **Status: Binnenkort Beschikbaar** — De SFTP API is gedefinieerd maar nog niet verbonden met de SFTP-engine van de app. `list()` retourneert momenteel een lege array, en `upload()`/`download()` zijn no-ops. Dit zal volledig geïmplementeerd worden in een toekomstige release. Voor nu, gebruik `ctx.terminal.send()` met `scp` of `rsync` commando's als een workaround.

#### `sftp.list(sessionId, path)`

Lijst bestanden in een externe directory.
```javascript
...
```
**Workaround (totdat SFTP API live is):**
```javascript
...
|-------|------|---------|-------------|
| `id` | `string` | Nee | Unieke ID (standaard naar plugin naam) |
| `icon` | `string` | Ja | Lucide icon naam (bijv., `'server'`, `'shield'`, `'database'`) |
| `label` | `string` | Ja | Knoptekst die in de zijbalk wordt weergegeven |
| `onClick` | `() => void` | Ja | Functie die wordt aangeroepen wanneer de knop wordt geklikt |
```javascript
context.ui.addSidebarButton({
  id: 'my-dashboard',
...
```
> Zie [Deel 3](#deel-3-bouwen-van-aangepaste-ui-met-webviews) voor geavanceerde webview-patronen.
...
| `message` | `string` | Tekst om weer te geven |
```javascript
...
| `text` | `string` | Tekst om weer te geven |
```javascript
...
> **Status: Binnenkort Beschikbaar** — De AI API is gedefinieerd maar nog niet verbonden met Soomy. Geeft momenteel `{ response: 'AI nog niet verbonden' }` terug. Volledige AI-integratie is gepland voor een toekomstige release.
...
## Deel 3: Bouwen van Aangepaste UI met Webviews

De `openWebview()` API stelt je in staat om dashboard UIs te bouwen met HTML, CSS en JavaScript — allemaal binnen een popupvenster.

> **Belangrijke beperking:** Webviews zijn **alleen-weergave**. Ze kunnen niet terugbellen naar plugin-API's (`ctx.settings`, `ctx.terminal`, enz.). Gebruik zijbalkknoppen voor alle gebruikersacties en gebruik `openWebview()` om de huidige staat weer te geven. Als je interactieve functies nodig hebt, activeer ze dan vanuit zijbalkknoppen en heropen de webview om de weergave te vernieuwen.
...
> **Opmerking:** Webviews zijn alleen-weergave — ze kunnen niet terugbellen naar plugin-API's. Gebruik `ctx.settings` in je zijbalkknophandlers om instellingen te wijzigen, en gebruik `openWebview()` om de huidige staat weer te geven.
```javascript
...
## Deel 4: Publiceren van je Plugin
...
| `description` | Eén-regel beschrijving | `"Monitors nginx access logs in real-time"` |
...
Deze regels zijn **verplicht**. Plugins die ze schenden zullen worden afgewezen.

| Regel | Waarom |
|-------|--------|
| **NOOIT** `eval()` of `new Function()` gebruiken | Risico op code-injectie |
| **NOOIT** `child_process`, `exec()`, `spawn()` gebruiken | Gebruik alleen `ctx.terminal.send()` voor commando's |
| **NOOIT** externe URL's ophalen | Uitzondering: `wiasoom.com` API-eindpunten |
| **NOOIT** `process.env` benaderen | Omgevingsvariabelen kunnen geheimen bevatten |
| **NOOIT** `require('fs')` direct gebruiken | Gebruik `ctx.settings` voor opslag, `ctx.sftp` voor bestandsoverdracht |
| **NOOIT** externe npm-pakketten gebruiken | Alleen pure JavaScript — geen node_modules |
...
Wikkel risicovolle bewerkingen altijd in try/catch:
```javascript
exports.activate = function(context) {
  try {
    // Your plugin logic
...
WIA SOOM ondersteunt 254 talen. Om jouw plugin-label vertaalbaar te maken, gebruik een eenvoudige aanpak:
```javascript
