...
<p align="center"><strong>Rakko du plugin 5 minuttas.</strong></p>
<p align="center">Luo voimakkaat palvelintyökalut, koontinäytöt ja automaatiot — suoraan WIA SOOM:ssa.</p>

---

## Sisällysluettelo

- [Osa 1: Nopeasti alkuun — Ensimmäinen plugin 5 minuutissa](#osa-1-nopeasti-alkuun--ensimmäinen-plugin-5-minuutissa)
- [Osa 2: Plugin Context API -viite](#osa-2-plugin-context-api--viite)
  - [ctx.terminal](#ctxterminal--suorita-komentoja-etäpalvelimilla)
  - [ctx.sftp](#ctxsftp--tiedostonsiirto)
  - [ctx.ui](#ctxui--käyttöliittymä)
  - [ctx.settings](#ctxsettings--pysyvä-tallennus)
  - [ctx.ai](#ctxai--ai-integraatio)
- [Osa 3: Mukautetun UI:n rakentaminen Webviewilla](#osa-3-mukautetun-uin-rakentaminen-webviewilla)
- [Osa 4: Plugin julkaiseminen](#osa-4-plugin-julkaiseminen)
...
## Osa 1: Nopeasti alkuun — Ensimmäinen plugin 5 minuutissa

### Mitä rakennat

"Hello World" plugin, joka lisää painikkeen sivupalkkiin. Kun sitä napsautetaan, se näyttää ilmoituksen.
...
Käynnistä sovellus uudelleen (tai vaihda plugin pois päältä/päälle Asetukset → Pluginit).

Sinun pitäisi nähdä **"Hello World"** painike sivupalkissa. Napsauta sitä — näet onnistumisilmoituksen!

### Kuinka se toimii
§§§CHUNK_SEPARATOR§§§
---

## Osa 2: Plugin Context API -viite

Kun `activate(context)`-funktiota kutsutaan, `context` (tai `ctx`) tarjoaa nämä API:t:
§§§CHUNK_SEPARATOR§§§
---
...
| `callback` | `(data: string) => void` | Kutsutaan jokaisella tulosteella |
...
> **Tila: Tulossa Pian** — SFTP API on määritelty, mutta ei vielä kytketty sovelluksen SFTP-moottoriin. `list()` palauttaa tällä hetkellä tyhjän taulukon, ja `upload()`/`download()` eivät tee mitään. Tämä toteutetaan täysin tulevassa julkaisussa. Tällä hetkellä käytä `ctx.terminal.send()` `scp` tai `rsync` komentoja kiertotienä.
...
|------------|--------|------------|--------|
| `id` | `string` | Ei | Uniikki ID (oletuksena pluginin nimi) |
| `icon` | `string` | Kyllä | Lucide-ikonin nimi (esim. `'server'`, `'shield'`, `'database'`) |
| `label` | `string` | Kyllä | Painiketeksti, joka näkyy sivupalkissa |
| `onClick` | `() => void` | Kyllä | Funktio, joka kutsutaan, kun painiketta napsautetaan |
§§§CHUNK_SEPARATOR§§§
**Ikoniviite:** Selaa kaikkia saatavilla olevia ikoneita osoitteessa [lucide.dev/icons](https://lucide.dev/icons)

> **Yhteensopivuus huomautus:** Jotkut vanhemmat pluginat käyttävät paikallisia argumentteja kuten `addSidebarButton(id, icon, label, onClick)`. Virallinen API käyttää **options-objektia** kuten yllä on dokumentoitu. Käytä aina objektityyliä uusille plugineille.

#### `ui.openWebview(options)`

Avaa ponnahdusikkuna mukautetulla HTML-sisällöllä. Näin rakennat rikkaita käyttöliittymiä.
...
> **Tila: Tulossa pian** — AI API on määritelty, mutta ei vielä liitetty Soomyyn. Palauttaa tällä hetkellä `{ response: 'AI not yet connected' }`. Täydellinen AI-integraatio on suunniteltu tulevaan julkaisuun.

#### `ai.chat(messages, options?)`

Lähetä viestejä AI-assistentille (Soomy).
```javascript
...
## Part 3: Rakentaminen Mukautetulla UI:lla Webviewien Kanssa

`openWebview()` API antaa sinun rakentaa kojelaudan UI:ta HTML:llä, CSS:llä ja JavaScriptillä — kaikki popup-ikkunassa.

> **Tärkeä rajoitus:** Webviewit ovat **vain näyttöä varten**. Ne eivät voi kutsua takaisin lisäosan API:hin (`ctx.settings`, `ctx.terminal`, jne.). Käytä sivupalkin painikkeita kaikille käyttäjätoimille ja käytä `openWebview()` näyttämään nykyinen tila. Jos tarvitset interaktiivisia ominaisuuksia, laukaise ne sivupalkin painikkeista ja avaa webview uudelleen näyttöä päivittääksesi.

### Malli: Terminallikomento → Tuloksen jäsentäminen → Näytä HTML:ssä
...
```
### Malli: Interaktiivinen Kojelauta Automaattisella Päivityksellä
```javascript
await context.sftp.upload(sessionId, '/tmp/config.json', '/etc/myapp/config.json');
context.ui.showNotification('success', 'Config uploaded!');
```
### Malli: Asetusten Näyttäminen Webviewissä

> **Huom:** Webviewit ovat vain näyttöä varten — ne eivät voi kutsua takaisin lisäosan API:hin. Käytä `ctx.settings` sivupalkin painikkeiden käsittelijöissä muuttaaksesi asetuksia, ja käytä `openWebview()` näyttämään nykyinen tila.
```javascript
await context.sftp.download(sessionId, '/var/log/app.log', '/tmp/app.log');
```
---

## Part 4: Lisäosan Julkaiseminen
...
4. Testaa äärimmäiset tapaukset: mitä tapahtuu, jos terminaalia ei ole liitetty?
...
```
**Vahkkus `package.json` -kenttä:**

| Kenttä | Kuvaus | Esimerkki |
|--------|--------|-----------|
| `name` | Ainulaatuinen kebab-case ID | `"my-awesome-plugin"` |
| `version` | Semanttinen versio | `"1.0.0"` |
| `description` | Yhden lauseen kuvaus | `"Seuraa nginx-pääsylokit reaaliajassa"` |
| `author` | Nimesi | `"John Doe"` |
| `main` | Alkuportti | `"index.js"` |
...
| `keywords` | Hakutagien taulukko |
| `soom.minVersion` | Vähimmäis WIA SOOM -version vaatimukset |
...
Nämä säännöt ovat **pakollisia**. Plugins, jotka rikkovat niitä, hylätään.
...
| **ÄLÄ KOSKAAN** käytä `require('fs')` suoraan | Käytä `ctx.settings` tallennukseen, `ctx.sftp` tiedonsiirtoon |
| **ÄLÄ KOSKAAN** käytä npm-ulkoisia paketteja | Pelkkä JavaScript vain — ei node_modules |
| **ON PAKKO** käyttää `ctx.terminal.send()` kaikille etäkomentoille | Tämä kulkee turvallisen SSH-kanavan kautta |
| **ON PAKKO** siivota `deactivate()` | Poista kuuntelijat, tyhjennä aikaväliä |

### Virheiden käsittely

Kä��ri aina riskialttiit toiminnot try/catch:
```javascript
context.ui.addSidebarButton({
  id: 'my-dashboard',
...
### Esimerkki 1: Palvelimen levytarkistin
...
> **Suunnittelumalli:** Koska webview't eivät voi suoraan kutsua plugin API:ita, tämä plugin käyttää "snapshot" -lähestymistapaa — se lukee TODO:t asetuksista, renderöi ne vain luku -HTML:nä ja tarjoaa sivupalkkipohjaisia toimintoja kohteiden lisäämiseksi. Webview on **näyttö**kerros, ei interaktiivinen lomake.
```javascript
...
Seuraa terminaalin ulostuloa ja lähettää ilmoituksen, kun tiettyjä kuvioita havaitaan.
```javascript
...
## Appendix: Kategoriijat & Ikonat

### Plugin Kategoriijat (29)

Bruk dii `package.json` `keywords` vai ku submitterat registrii:

| Kategoriija | Beskrivvuo |
|-------------|------------|
| `server` | Gáddje serveriid |
| `devtools` | Dálkkáldat náhkkáldat |
| `calculator` | Kalkulatorat ja muhtimát |
| `simulator` | Simulátorat |
| `game` | Terminalmát |
| `business` | Busiess náhkkáldat |
| `security` | Turvva ja auditereing |
| `web` | Web serveriid |
| `education` | Oahpahus náhkkáldat |
| `health` | Dássehuvva náhkkáldat |
| `islamic` | Islamala náhkkáldat (márkanat, jne.) |
| `science` | Sciensá náhkkáldat |
| `quantum` | Kvantum computera náhkkáldat |
| `ai` | AI-válddá náhkkáldat |
| `biotech` | Bioteknologiija náhkkáldat |
| `space` | Álggu ja astronomii náhkkáldat |
| `network` | Verkkonáhkkáldat |
| `database` | Databásá gávdnji |
| `monitoring` | Server monitoring |
| `devops` | DevOps ja CI/CD |
| `utility` | Gáddje náhkkáldat |
| `design` | Dássehuvva náhkkáldat |
| `ecommerce` | E-kauppa náhkkáldat |
| `automation` | Automatiserij náhkkáldat |
| `kpop` | K-pop válddá náhkkáldat |
| `accessibility` | Ákkesvuohta náhkkáldat |
| `analytics` | Analytikka ja raporteering |
| `wia` | WIA ekosystema náhkkáldat |
| `all` | Dássá all kategoriijat |

### Válggo Ikonat (Lucide)

| Ikona Návnna | Bruk til |
|---------------|----------|
| `server` | Server gávdnji |
| `shield` | Turvva |
| `database` | Databásá |
| `activity` | Monitoring |
| `terminal` | Terminal náhkkáldat |
| `code` | Dálkkáldat |
| `hard-drive` | Disk/storage |
| `network` | Verkkonáhkkáldat |
| `lock` | Auth/encryption |
| `eye` | Dálkkáldat/monitoring |
| `check-square` | Ákkes/TODO |
| `layout-dashboard` | Dashboards |
| `settings` | Konfiguráhtio |
| `zap` | Automatiserij |
| `globe` | Web/internasjonála |

Browza all 1,500+ ikonat: [lucide.dev/icons](https://lucide.dev/icons)
...
<p align="center"><em>Rakna dii nuorra. Dálkkáldat máŋga.</em></p>
