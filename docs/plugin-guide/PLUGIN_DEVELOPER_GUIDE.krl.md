...
<h1 align="center">WIA SOOM Plugin Kehittäjäopas</h1>
<p align="center"><strong>Rakenna oma pluginisi 5 minuutissa.</strong></p>
<p align="center">Luo tehokkaita palvelintyökaluja, hallintapaneeleja ja automaatioita — suoraan WIA SOOM:issa.</p>
...
- [Osa 3: Mukautetun käyttöliittymän rakentaminen Webviewien avulla](#osa-3-mukautetun-käyttöliittymän-rakentaminen-webviewien-avulla)
- [Osa 4: Pluginisi julkaiseminen](#osa-4-pluginisi-julkaiseminen)
- [Osa 5: Parhaat käytännöt](#osa-5-parhaat-käytännöt)
- [Osa 6: Reaalimaailman esimerkit](#osa-6-reaalimaailman-esimerkit)
...
Käynnistä sovellus uudelleen (tai vaihda plugin pois päältä/päälle Asetukset → Lisäosat).

Sinun pitäisi nähdä **"Hello World"** painike sivupalkissa. Klikkaa sitä — näet onnistumisilmoituksen!
...
Lisää painike WIA SOOM sivupalkkiin.
...
> **Yhteensopivuus huomautus:** Jotkut vanhemmat lisäosat käyttävät paikkatietoja, kuten `addSidebarButton(id, icon, label, onClick)`. Virallinen API käyttää **vaihtoehto-objektia** kuten yllä on dokumentoitu. Käytä aina objektityyliä uusille lisäosille.
...
```
> Katso [Osa 3](#part-3-building-custom-ui-with-webviews) edistyneitä webview-malleja varten.
...
| Parametri | Tyyppi | Kuvaus |
|-----------|--------|--------|
| `id` | `string` | Yksilöllinen ID tälle tilaelementille |
...
Pluginin asetukset tallennetaan pysyvästi `~/.wia-soom/plugins/{your-plugin}/.plugin-settings.json`-tiedostoon.
...
`openWebview()` API mahdollistaa hallintapaneelin UI:den rakentamisen HTML:n, CSS:n ja JavaScriptin avulla — kaikki popup-ikkunassa.

> **Tärkeä rajoitus:** Webview't ovat **näyttöön liittyviä**. Ne eivät voi kutsua takaisin plugin API:ita (`ctx.settings`, `ctx.terminal`, jne.). Käytä sivupalkin painikkeita kaikissa käyttäjätoiminnoissa ja käytä `openWebview()` nykytilan näyttämiseen. Jos tarvitset interaktiivisia ominaisuuksia, aktivoi ne sivupalkin painikkeista ja avaa webview uudelleen näyttöä päivittääksesi.

### Malli: Terminal-komento → Tuloksen jäsentäminen → Näytä HTML:ssä

Tämä on yleisin plugin-malli. Suoritat komennon, jäsentät tuloksen ja näytät sen visuaalisesti.
```javascript
...
> **Huom:** Webview't ovat näyttöön liittyviä — ne eivät voi kutsua takaisin plugin API:ita. Käytä `ctx.settings` sivupalkin painikehandlerissa muuttaaksesi asetuksia, ja käytä `openWebview()` näyttämään nykytila.
```javascript
...
## Osa 4: Pluginisi julkaiseminen

### Vaihe 1: Testaa paikallisesti

1. Kopioi pluginisi `~/.wia-soom/plugins/{your-plugin}/`
...
Plugin-kansiosi on sisällettävä:
```javascript
...
| **ON** käytettävä `ctx.terminal.send()` kaikissa etäkomentoissa | Tämä kulkee turvallisen SSH-kanavan kautta |
| **ON** siivottava `deactivate()`-funktiossa | Poista kuuntelijat, tyhjennä aikaväliä |
...
Valvoo terminaalin ulostuloa ja lähettää ilmoituksen, kun tietyt mallit havaitaan.
```javascript
...
Käytä näitä `package.json` `keywords` -kentässä tai rekisteriin lähettämisessä:
...
| `ecommerce` | Verkkokauppatyökalut |
...
| `zap` | Automaatio |
| `globe` | Verkkointernationaalinen |
...
- **Plugin Ongelmat:** [GitHub Issues](https://github.com/WIA-Official/wiasoom.com/issues)
- **Esimerkkiplugin:** [Website](https://wiasoom.com)
