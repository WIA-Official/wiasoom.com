...
Cuando tu función `activate(context)` es llamada, `context` (o `ctx`) proporciona estas APIs:
```typescript
...
Abre una ventana emergente con contenido HTML personalizado. Así es como construyes interfaces ricas.
...
| **NUNCA** obtener URLs externas | Excepción: endpoints de la API de `wiasoom.com` |
...
Utiliza estos en tu `package.json` `keywords` o al enviar a el registro:
...
<p align="center"><em>Construye algo increíble. Compártelo con el mundo.</em></p>
