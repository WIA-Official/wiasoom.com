...
<h1 align="center">Guida pe' Sviluppatori di Plugin WIA SOOM</h1>
<p align="center"><strong>Crea 'o tuo plugin in 5 minuti.</strong></p>
...
- [Parte 1: Inizio Veloce — 'O Tuo Primo Plugin in 5 Minuti](#parte-1-inizio-veloce--o-tuo-primo-plugin-in-5-minuti)
- [Parte 2: Riferimento API Contexto Plugin](#parte-2-riferimento-api-contesto-plugin)
  ...
  - [ctx.settings](#ctxsettings--memoria-permanente)
...
- [Parte 4: Pubblicare 'O Tuo Plugin](#parte-4-pubblicare-o-tuo-plugin)
...
Riavvia l'app (o attiva/disattiva 'o plugin nelle Impostazioni → Plugin).
...
## Parte 2: Riferimento API Contexto Plugin

Quando 'a funzione `activate(context)` è chiamata, `context` (o `ctx`) fornisce ste API:
```typescript
...
|-----------|------|-------------|
| `sessionId` | `string` | 'A sessione terminale a cui inviare |
| `data` | `string` | 'O comando o dato da inviare |
```javascript
...
| Parametro | Tipo | Descrizione |
|-----------|------|-------------|
| `sessionId` | `string` | 'A sessione terminale da osservare |
| `callback` | `(data: string) => void` | Chiamato con ogni pezzo di output |
...
```
**Soluzione alternativa (fino a quando l'API SFTP è attiva):**
```javascript
...
|---------|------|-----------|-------------|
| `id` | `string` | No | ID unico (di default è 'o nome do' plugin) |
| `icon` | `string` | Sì | Nome icona Lucide (es., `'server'`, `'shield'`, `'database'`) |
| `label` | `string` | Sì | Testo do' pulsante mostrato nella barra laterale |
| `onClick` | `() => void` | Sì | Funzione chiamata quando 'o pulsante è cliccato |
```javascript
context.ui.addSidebarButton({
  id: 'my-dashboard',
...
```
**Riferimento icona:** Sfoglia tutte le icone disponibili su [lucide.dev/icons](https://lucide.dev/icons)
...
```
> Vedi [Parte 3](#part-3-building-custom-ui-with-webviews) pe' schemi avanzati di webview.

#### `ui.showNotification(type, message)`

Mostra una notifica a toast.
...
> **Stato: In Arrivo** — L'API AI è definita ma non ancora connessa a Soomy. Attualmente restituisce `{ response: 'AI not yet connected' }`. L'integrazione completa dell'AI è pianificata per un rilascio futuro.
...
L'API `openWebview()` ti consente di costruire interfacce di dashboard con HTML, CSS e JavaScript — tutto dentro una finestra popup.

> **Limitazione importante:** Le webviews sono **solo per visualizzazione**. Non possono richiamare le API del plugin (`ctx.settings`, `ctx.terminal`, ecc.). Usa i pulsanti della barra laterale per tutte le azioni dell'utente e usa `openWebview()` per mostrare lo stato attuale. Se hai bisogno di funzionalità interattive, attivale dai pulsanti della barra laterale e riapri la webview per aggiornare la visualizzazione.
...
> **Nota:** Le webviews sono solo per visualizzazione — non possono richiamare le API del plugin. Usa `ctx.settings` nei gestori dei pulsanti della barra laterale per modificare le impostazioni e usa `openWebview()` per mostrare lo stato attuale.
§§§CHUNK_SEPARATOR§§§
---
...
4. Testa i casi limite: cosa succede se non è connesso alcun terminale?

### Passo 2: Preparati per la sottomissione
...
### Passo 3: Invia al Plugin Registry
...
### Esempio 1: Controllo Disco del Server
