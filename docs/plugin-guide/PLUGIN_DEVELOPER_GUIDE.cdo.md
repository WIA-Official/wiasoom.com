...
> **状态：即将推出** — SFTP API 已定义，但尚未连接到应用的 SFTP 引擎。`list()` 当前返回一个空数组，而 `upload()`/`download()` 是无操作。这将在未来的版本中完全实现。现在，请使用 `ctx.terminal.send()` 和 `scp` 或 `rsync` 命令作为解决方法。
...
> **兼容性说明：** 一些旧插件使用位置参数，如 `addSidebarButton(id, icon, label, onClick)`。官方 API 使用如上所述的 **选项对象**。对于新插件，请始终使用对象样式。
...
> **注意：** Webviews 只能顯示 — 它們不能調用插件 API。使用 `ctx.settings` 在你的側邊按鈕處理程序中修改設置，並使用 `openWebview()` 來顯示當前狀態。
```javascript
...
| **绝对不要** 直接使用 `require('fs')` | 使用 `ctx.settings` 进行存储，使用 `ctx.sftp` 进行文件传输 |
...
一个使用设置进行持久存储并使用 webview 显示的 TODO 列表管理插件。

> **设计模式：** 由于 webviews 不能直接调用插件 API，因此该插件使用“快照”方法 — 它从设置中读取 TODO，将其渲染为只读 HTML，并提供基于侧边栏的操作来添加项目。webview 是一个 **显示** 层，而不是交互式表单。
```javascript
'use strict';
