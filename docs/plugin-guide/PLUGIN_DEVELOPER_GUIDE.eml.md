...
<h1 align="center">Guida per i Sviluppatori di Plugin di WIA SOOM</h1>
<p align="center"><strong>Crea il tuo plugin in 5 minuti.</strong></p>
<p align="center">Crea strumenti server potenti, dashboard e automazioni — direttamente all'interno di WIA SOOM.</p>
...
| `html` | `string` | Contenuto HTML completo da rendere |
```javascript
context.ui.addSidebarButton({
  id: 'my-dashboard',
...
> **Stato: In Arrivo** — L'API AI è definita ma non ancora collegata a Soomy. Attualmente restituisce `{ response: 'AI not yet connected' }`. È prevista un'integrazione completa dell'AI per una futura release.
...
> **Nota:** Le webviews sono solo in visualizzazione — non possono richiamare le API del plugin. Usa `ctx.settings` nei gestori dei pulsanti della barra laterale per modificare le impostazioni e usa `openWebview()` per mostrare lo stato attuale.
```javascript
exports.activate = function(context) {
  context.ui.addSidebarButton({
    icon: 'hard-drive',
...
4. Testa i casi limite: cosa succede se non è collegato alcun terminale?
...
### Passo 3: Invia al Registro dei Plugin
...
2. Un pacchetto ZIP viene creato in `dist/`
