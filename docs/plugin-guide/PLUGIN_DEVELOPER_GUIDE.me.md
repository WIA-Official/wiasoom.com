...
<p align="center">Kreirajte moćne alate za servere, kontrolne table i automatizacije — direktno unutar WIA SOOM.</p>
...
- [Deo 3: Izrada prilagođenog UI-a sa Webview-ima](#deo-3-izrada-prilagođenog-ui-a-sa-webview-ima)
...
"Dodatak 'Hello World'" koji dodaje dugme u bočnu traku. Kada se klikne, prikazuje obaveštenje.
...
Kada se vaša `activate(context)` funkcija pozove, `context` (ili `ctx`) pruža ove API-je:
§§§CHUNK_SEPARATOR§§§
---
...
Pretplatite se na sve izlaze iz terminal sesije. Vraća **funkciju za odjavu**.
...
§§§CHUNK_SEPARATOR§§§
**Važno:** Uvek sačuvajte funkciju za odjavu i pozovite je u `deactivate()` da sprečite curenje memorije.

---

### `ctx.sftp` — Prenos datoteka

> **Status: Uskoro** — SFTP API je definisan, ali još nije povezan sa SFTP motorom aplikacije. `list()` trenutno vraća prazan niz, a `upload()`/`download()` su neaktivni. Ovo će biti potpuno implementirano u budućem izdanju. Za sada, koristite `ctx.terminal.send()` sa `scp` ili `rsync` komandama kao rešenje.
...
§§§CHUNK_SEPARATOR§§§
**Referenca ikona:** Pretražite sve dostupne ikone na [lucide.dev/icons](https://lucide.dev/icons)

> **Napomena o kompatibilnosti:** Neki stariji dodaci koriste pozicijske argumente kao što su `addSidebarButton(id, icon, label, onClick)`. Zvanični API koristi **objekat opcija** kao što je dokumentovano iznad. Uvek koristite stil objekta za nove dodatke.

#### `ui.openWebview(options)`

Otvorite iskačući prozor sa prilagođenim HTML sadržajem. Ovo je način na koji gradite bogate korisničke interfejse.
...
| `message` | `string` | Tekst koji treba prikazati |
```json
{
  "name": "hello-world",
  "version": "1.0.0",
...
| `text` | `string` | Tekst koji treba prikazati |
```javascript
'use strict';
...
```
**Primer: Zapamti korisničke preferencije**
```javascript
...
API `openWebview()` vam omogućava da izgradite UI-eve kontrolne table koristeći HTML, CSS i JavaScript — sve unutar iskačućeg prozora.

> **Važno ograničenje:** Webview-i su **samo za prikaz**. Ne mogu se pozivati nazad u API-je plugina (`ctx.settings`, `ctx.terminal`, itd.). Koristite dugmad u bočnoj traci za sve korisničke akcije, i koristite `openWebview()` da prikažete trenutni status. Ako su vam potrebne interaktivne funkcije, aktivirajte ih iz dugmadi u bočnoj traci i ponovo otvorite webview da osvežite prikaz.
...
```
### Obrazac: Interaktivna Kontrolna Tabla sa Automatskim Osvežavanjem
```javascript
...
> **Napomena:** Webview-i su samo za prikaz — ne mogu se pozivati nazad u API-je plugina. Koristite `ctx.settings` u handler-ima dugmadi u bočnoj traci da modifikujete podešavanja, i koristite `openWebview()` da prikažete trenutni status.
```javascript
...
3. Proverite da li radi: dugme u bočnoj traci se pojavljuje, funkcije rade ispravno
...
| **MORATE** koristiti `ctx.terminal.send()` za sve daljinske komande | Ovo ide kroz siguran SSH kanal |
...
Pokreće `df -h` na udaljenom serveru i prikazuje korišćen prostor/dostupni prostor u statusnoj traci.
```javascript
...
Plugin koji upravlja TODO listom koristeći podešavanja za trajno skladištenje i webview za prikaz.

> **Dizajn obrazac:** Pošto webview ne može direktno pozivati plugin API-je, ovaj plugin koristi pristup "snapshot" — čita TODO-e iz podešavanja, prikazuje ih kao HTML samo za čitanje, i pruža akcije zasnovane na bočnoj traci za dodavanje stavki. Webview je **prikaz** sloj, a ne interaktivni oblik.
```javascript
