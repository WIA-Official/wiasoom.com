...
<p align="center">Cipta alat server yang berkuasa, papan pemuka, dan automasi — terus dalam WIA SOOM.</p>

---

## Jadual Kandungan

- [Bahagian 1: Permulaan Pantas — Plugin Pertama Anda dalam 5 Minit](#bahagian-1-permulaan-pantas--plugin-pertama-anda-dalam-5-minit)
- [Bahagian 2: Rujukan API Konteks Plugin](#bahagian-2-rujukan-api-konteks-plugin)
  - [ctx.terminal](#ctxterminal--jalankan-perintah-di-server-jauh)
  - [ctx.sftp](#ctxsftp--pemindahan-fail)
  - [ctx.ui](#ctxui--antaramuka-pengguna)
  - [ctx.settings](#ctxsettings--penyimpanan-berterusan)
  - [ctx.ai](#ctxai--integrasi-ai)
- [Bahagian 3: Membangun UI Kustom dengan Webviews](#bahagian-3-membangun-ui-kustom-dengan-webviews)
- [Bahagian 4: Menerbitkan Plugin Anda](#bahagian-4-menerbitkan-plugin-anda)
- [Bahagian 5: Amalan Terbaik](#bahagian-5-amalan-terbaik)
- [Bahagian 6: Contoh Dunia Sebenar](#bahagian-6-contoh-dunia-sebenar)
...
Mulakan semula aplikasi (atau togol plugin off/on dalam Tetapan → Plugins).

Anda seharusnya melihat butang **"Hello World"** di sidebar. Klik ia — anda akan melihat notifikasi kejayaan!
...
### `ctx.terminal` — Jalankan perintah di server jauh

#### `terminal.send(sessionId, data)`

Hantar perintah (atau sebarang data) ke sesi terminal yang aktif.

| Parameter | Jenis | Penerangan |
|-----------|------|-------------|
| `sessionId` | `string` | Sesi terminal untuk dihantar |
| `data` | `string` | Perintah atau data untuk dihantar |
```javascript
...
Langgan semua output dari sesi terminal. Mengembalikan **fungsi batal langganan**.

| Parameter | Jenis | Penerangan |
|-----------|------|-------------|
| `sessionId` | `string` | Sesi terminal untuk diperhatikan |
...
unsubscribe();
```
**Penting:** Sentiasa simpan fungsi batal langganan dan panggil ia dalam `deactivate()` untuk mengelakkan kebocoran memori.

---

### `ctx.sftp` — Pemindahan fail

> **Status: Akan Datang** — API SFTP telah ditakrifkan tetapi belum disambungkan ke enjin SFTP aplikasi. `list()` kini mengembalikan array kosong, dan `upload()`/`download()` tidak berfungsi. Ini akan dilaksanakan sepenuhnya dalam keluaran akan datang. Untuk sekarang, gunakan `ctx.terminal.send()` dengan perintah `scp` atau `rsync` sebagai penyelesaian sementara.
...
Muat naik fail dari mesin tempatan ke server jauh.
```javascript
...
Muat turun fail dari server jauh ke mesin tempatan.
```javascript
...
|--------|------|----------|-------------|
| `id` | `string` | Tidak | ID unik (default kepada nama plugin) |
...
```
**Rujukan ikon:** Lihat semua ikon yang tersedia di [lucide.dev/icons](https://lucide.dev/icons)
...
| `html` | `string` | Kandungan HTML penuh untuk dirender |
```javascript
context.ui.openWebview({
  title: 'Server Status',
  html: `
...
Tunjukkan pemberitahuan toast.

| Parameter | Jenis | Penerangan |
|-----------|------|-------------|
| `type` | `'success' \| 'error' \| 'info'` | Gaya pemberitahuan |
...
> **Status: Akan Datang** — API AI telah ditakrifkan tetapi belum disambungkan ke Soomy. Pada masa ini mengembalikan `{ response: 'AI not yet connected' }`. Integrasi AI penuh dirancang untuk keluaran akan datang.
...
4. Uji kes tepi: apa yang berlaku jika tiada terminal disambungkan?
...
2. Bundel ZIP dicipta dalam `dist/`
3. Plugin anda muncul dalam **Plugin Store** untuk semua pengguna WIA SOOM!
...
| **WAJIB** gunakan `ctx.terminal.send()` untuk semua arahan jauh | Ini melalui saluran SSH yang selamat |
| **WAJIB** membersihkan dalam `deactivate()` | Buang pendengar, kosongkan selang |
...
## Bahagian 6: Contoh Dunia Sebenar

### Contoh 1: Pemeriksa Disk Pelayan

Menjalankan `df -h` pada pelayan jauh dan menunjukkan ruang yang digunakan/tersedia dalam bar status.
```javascript
'use strict';
...
> **Corak reka bentuk:** Oleh kerana webview tidak boleh memanggil API plugin secara langsung, plugin ini menggunakan pendekatan "snapshot" — ia membaca TODO dari tetapan, merendernya sebagai HTML hanya-baca, dan menyediakan tindakan berasaskan sidebar untuk menambah item. Webview adalah lapisan **paparan**, bukan borang interaktif.
```javascript
'use strict';
