...
<p align="center"><strong>Jab in aolep plugin in 5 minit.</strong></p>
<p align="center">Jab in powerful server tools, dashboards, im automations — im WIA SOOM.</p>
...
```
> Ejjab [Part 3](#part-3-building-custom-ui-with-webviews) ejjeḷḷọk wōt wōt in webview.

#### `ui.showNotification(type, message)`

Ejjab wōt notification.

| Parameter | Type | Description |
|-----------|------|-------------|
| `type` | `'success' \| 'error' \| 'info'` | Wōt notification |
| `message` | `string` | Kōmān ejjeḷḷọk |
```javascript
...
Ejjab wōt text item eo ejjeḷḷọk in bar status eo an.

| Parameter | Type | Description |
|-----------|------|-------------|
| `id` | `string` | ID jān in status item |
| `text` | `string` | Kōmān ejjeḷḷọk |
```javascript
...
### `ctx.settings` — Wōt storage

Ejjab wōt plugin settings eo ejjeḷḷọk in `~/.wia-soom/plugins/{your-plugin}/.plugin-settings.json`.

#### `settings.get(key)`

Ejjab wōt value eo ejjeḷḷọk.
```javascript
var theme = context.settings.get('theme');       // 'dark'
var count = context.settings.get('refreshRate'); // 30
var items = context.settings.get('todoList');    // [{ text: '...', done: false }, ...]
```
Ejjab `undefined` in ke key eḷap.

#### `settings.set(key, value)`

Ejjab wōt value. Ejjab wōt strings, numbers, booleans, arrays, im objects.
```javascript
...
```
**Example: Kōmān ejjeḷḷọk user preferences**
```javascript
...
> **Status: Coming Soon** — Ejjab wōt AI API eo ejjeḷḷọk ak eḷap jān Soomy. Ejjab wōt `{ response: 'AI not yet connected' }`. Ejjab wōt full AI integration eo ejjeḷḷọk in wōt jān.

#### `ai.chat(messages, options?)`

Ejjab wōt messages jān in AI assistant (Soomy).
```javascript
...
## Part 3: Kōmān Custom UI jān Webviews

Ejjab wōt `openWebview()` API eo ejjeḷḷọk in build dashboard UIs jān HTML, CSS, im JavaScript — aolep in wōt popup window.

> **Important limitation:** Ejjab wōt webviews **display-only**. Ejjab wōt call back in plugin APIs (`ctx.settings`, `ctx.terminal`, etc.). Ejjab wōt sidebar buttons jān aolep user actions, im ejjab wōt `openWebview()` jān kōmān current state. Jān ke ejjeḷḷọk interactive features, ejjab wōt in sidebar buttons im re-open wōt webview jān refresh wōt display.

### Pattern: Terminal Command → Parse Output → Show in HTML

Ejjab wōt plugin pattern eo ejjeḷḷọk in. Ejjab wōt command, parse wōt result, im ejjeḷḷọk in visually.
```javascript
exports.activate = function(context) {
  context.ui.addSidebarButton({
    icon: 'hard-drive',
...
```
### Pattern: Interactive Dashboard jān Auto-Refresh
```javascript
exports.activate = function(context) {
  context.ui.addSidebarButton({
    icon: 'activity',
...
```
### Pattern: Kōmān Settings in wōt Webview

> **Note:** Ejjab wōt webviews ejjeḷḷọk — eḷap jān call back in plugin APIs. Ejjab wōt `ctx.settings` in aolep sidebar button handlers jān modify settings, im ejjab wōt `openWebview()` jān kōmān current state.
```javascript
...
## Part 4: Kōmān Plugin eo

### Step 1: Test locally

1. Kōmān wōt plugin eo jān `~/.wia-soom/plugins/{your-plugin}/`
2. Restart WIA SOOM
3. Verify eḷap wōt: sidebar button eḷap, features eḷap wōt
4. Test edge cases: eḷap jān ke eḷap terminal eḷap connected?

### Step 2: Kōmān submission

Ejjab wōt plugin folder eo ejjeḷḷọk:
```
your-plugin/
