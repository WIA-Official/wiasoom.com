...
<p align="center"><strong>Crea el teu propi plugin en 5 minutos.</strong></p>
<p align="center">Crea potentes ferramentes de servidor, tableros i automatizaciones — directamente dentro de WIA SOOM.</p>

---

## Taula de Contidos

- [Parte 1: Comenzar Rápido — Tu Primer Plugin en 5 Minutos](#parte-1-comenzar-rápido--tu-primer-plugin-en-5-minutos)
- [Parte 2: Referencia de la API del Contexto del Plugin](#parte-2-referencia-de-la-api-del-contexto-del-plugin)
  ...
  - [ctx.sftp](#ctxsftp--transferencia-de-arxius)
  - [ctx.ui](#ctxui--interfaz-de-usuariu)
  - [ctx.settings](#ctxsettings--almacenamiento-persistente)
  - [ctx.ai](#ctxai--integración-de-ai)
...
## Parte 1: Comenzar Rápido — Tu Primer Plugin en 5 Minutos
...
```
**Solución alternativa (hasta que la API de SFTP esté activa):**
```javascript
...
```
> Vea [Parte 3](#part-3-building-custom-ui-with-webviews) para patrones avanzados de webview.
...
| Parámetro | Tipo | Descripción |
|-----------|------|-------------|
| `type` | `'success' \| 'error' \| 'info'` | Estilo de la notificación |
...
La configuración del plugin se almacena de forma permanente en `~/.wia-soom/plugins/{your-plugin}/.plugin-settings.json`.
...
> **Estado: Próximamente** — La API de IA está definida pero aún no está conectada a Soomy. Actualmente devuelve `{ response: 'AI not yet connected' }`. Se planea una integración completa de IA para una futura versión.
...
> **Limitación importante:** Los webviews son **solo de visualización**. No pueden llamar a las APIs del plugin (`ctx.settings`, `ctx.terminal`, etc.). Usa botones en la barra lateral para todas las acciones del usuario, y usa `openWebview()` para mostrar el estado actual. Si necesitas características interactivas, actívalas desde los botones de la barra lateral y vuelve a abrir el webview para actualizar la visualización.
...
```
### Patrón: Panel de Control Interactivo con Auto-Actualización
```javascript
exports.activate = function(context) {
  context.ui.addSidebarButton({
    icon: 'activity',
...
> **Nota:** Los webviews son solo de visualización — no pueden llamar a las APIs del plugin. Usa `ctx.settings` en los manejadores de botones de la barra lateral para modificar configuraciones, y usa `openWebview()` para mostrar el estado actual.
```javascript
...
2. Reinicia WIA SOOM
3. Verifica que funcione: el botón de la barra lateral aparece, las funciones funcionan correctamente
4. Prueba casos extremos: ¿qué pasa si no hay terminal conectada?

### Paso 2: Prepárate para la presentación
...
| `description` | Descripción en una línea | `"Monitora los registros de acceso de nginx en tiempo real"` |
...
2. **Añadir** tu plugin a `plugins/{tu-nombre-de-plugin}/`
...
## Apéndice: Categoríes & Iconos

### Categoríes de Plugins (29)

Utiliza estos en tu `package.json` `keywords` o al enviar a la registru:

| Categoría | Descripción |
|-----------|-------------|
| `server` | Xestión xeneral del servidor |
| `devtools` | Ferramientes de desendolcu |
| `calculator` | Calculadores y convertidores |
| `simulator` | Simuladores |
| `game` | Xuegos de terminal |
| `business` | Ferramientes pa negocios |
| `security` | Seguridá y auditoría |
| `web` | Xestión del servidor web |
| `education` | Ferramientes educatives |
| `health` | Ferramientes relacionaes con la salú |
| `islamic` | Ferramientes islámicas (horarios de oración, etc.) |
| `science` | Ferramientes científiques |
| `quantum` | Ferramientes de computación cuántica |
| `ai` | Ferramientes impulsades por IA |
| `biotech` | Ferramientes de biotecnoloxía |
| `space` | Ferramientes del espaciu y astronomía |
| `network` | Ferramientes de rede |
| `database` | Xestión de bases de datos |
| `monitoring` | Monitorización del servidor |
| `devops` | DevOps y CI/CD |
| `utility` | Utilidaes xenerales |
| `design` | Ferramientes de deseñu |
| `ecommerce` | Ferramientes de comercio electrónicu |
| `automation` | Ferramientes d'automatización |
| `kpop` | Ferramientes relacionaes con K-pop |
| `accessibility` | Ferramientes d'accessibilidá |
| `analytics` | Analítiques y reportes |
| `wia` | Ferramientes del ecosistema WIA |
| `all` | Aparez en toles categoríes |

### Iconos Recomendados (Lucide)

| Nome del Icono | Usar pa |
|----------------|---------|
| `server` | Xestión del servidor |
| `shield` | Seguridá |
| `database` | Base de datos |
| `activity` | Monitorización |
| `terminal` | Ferramientes de terminal |
| `code` | Desendolcu |
| `hard-drive` | Disco/almacenamientu |
| `network` | Redes |
...
Explora tolos 1,500+ iconos: [lucide.dev/icons](https://lucide.dev/icons)

---

## Necesitas Ayuda?

- **Problemes en GitHub:** [wia-soom/issues](https://github.com/WIA-Official/wiasoom.com/issues)
- **Problemes de Plugins:** [GitHub Issues](https://github.com/WIA-Official/wiasoom.com/issues)
- **Plugins d'Exemplu:** [Website](https://wiasoom.com)
- **Sitio Web:** [wiasoom.com](https://wiasoom.com)

---

<p align="center"><em>Construye algo asombrosu. Comparteo con el mundu.</em></p>
<p align="center"><em>— El Equipu de WIA SOOM</em></p>