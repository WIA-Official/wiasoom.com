...
- [Teil 3: Erstellung einer benutzerdefinierten UI mit Webviews](#teil-3-erstellung-einer-benutzerdefinierten-ui-mit-webviews)
- [Teil 4: Veröffentlichung Ihres Plugins](#teil-4-veröffentlichung-ihres-plugins)
...
Ein "Hello World" Plugin, das einen Button zur Seitenleiste hinzufügt. Wenn darauf geklickt wird, zeigt es eine Benachrichtigung an.
...
```
### Schritt 4: Starten Sie WIA SOOM neu

Starten Sie die App neu (oder schalten Sie das Plugin in den Einstellungen → Plugins aus/ein).

Sie sollten einen **"Hello World"** Button in der Seitenleiste sehen. Klicken Sie darauf — Sie sehen eine Erfolgsmeldung!
...
Wenn Ihre `activate(context)` Funktion aufgerufen wird, bietet `context` (oder `ctx`) diese APIs:
```typescript
...
|-----------|------|-------------|
| `sessionId` | `string` | Die Terminal-Sitzung, an die gesendet wird |
| `data` | `string` | Der Befehl oder die Daten, die gesendet werden |
```javascript
...
| **Gibt zurück** | `() => void` | Rufen Sie dies auf, um das Zuhören zu beenden |
```javascript
...
unsubscribe();
```
**Wichtig:** Speichern Sie immer die Abmeldefunktion und rufen Sie sie in `deactivate()` auf, um Speicherlecks zu verhindern.
...
```
**Workaround (bis die SFTP API verfügbar ist):**
```javascript
...
Fügen Sie einen Button zur WIA SOOM Seitenleiste hinzu.

| Option | Typ | Erforderlich | Beschreibung |
|--------|------|--------------|-------------|
| `id` | `string` | Nein | Eindeutige ID (Standard ist der Plugin-Name) |
...
Öffnen Sie ein Popup-Fenster mit benutzerdefiniertem HTML-Inhalt. So erstellen Sie reichhaltige UIs.
...
> **Status: Kommt Bald** — Die KI-API ist definiert, aber noch nicht mit Soomy verbunden. Gibt derzeit `{ response: 'AI not yet connected' }` zurück. Eine vollständige KI-Integration ist für eine zukünftige Version geplant.
...
4. Testen Sie Randfälle: Was passiert, wenn kein Terminal verbunden ist?
...
| `soom.minVersion` | Minimale erforderliche WIA SOOM-Version |
...
Diese Regeln sind **verpflichtend**. Plugins, die gegen diese verstoßen, werden abgelehnt.

| Regel | Warum |
|------|-----|
| **NIE** `eval()` oder `new Function()` verwenden | Risiko der Code-Injektion |
| **NIE** `child_process`, `exec()`, `spawn()` verwenden | Verwenden Sie nur `ctx.terminal.send()` für Befehle |
...
| **NIE** `require('fs')` direkt verwenden | Verwenden Sie `ctx.settings` für Speicher, `ctx.sftp` für Dateiübertragungen |
...
Verwenden Sie diese in Ihrem `package.json` `keywords` oder beim Einreichen im Registry:
