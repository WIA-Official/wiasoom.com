...
Deberías ver un **"Hola Mundo"** botón en la barra lateral. ¡Haz clic en él y verás una notificación de éxito!
...
```
> Ve [Parte 3](#part-3-building-custom-ui-with-webviews) ta patrones avanzados de webview.

#### `ui.showNotification(type, message)`

Muestra una notificación tipo toast.

| Paràmetro | Tipo | Descripción |
|-----------|------|-------------|
...
Añade un ítem de texto persistente a la barra de estado inferior.
...
Las configuraciones del plugin se almacenan permanentemente en `~/.wia-soom/plugins/{your-plugin}/.plugin-settings.json`.
...
> **Nota:** Los webviews son solo de visualización — no pueden llamar de vuelta a las APIs del plugin. Usa `ctx.settings` en tus manejadores de botones de la barra lateral para modificar configuraciones, y usa `openWebview()` para mostrar el estado actual.
```javascript
exports.activate = function(context) {
  context.ui.addSidebarButton({
    icon: 'activity',
...
```
**Campos requerids en `package.json`:**
...
| **DEBES** usar `ctx.terminal.send()` para todos los comandos remotos | Esto pasa por el canal SSH seguro |
...
Un plugin que gestiona una lista de TODO utilizando configuraciones para almacenamiento persistente y un webview para la visualización.

> **Patrón de diseño:** Dado que los webviews no pueden llamar directamente a las APIs de los plugins, este plugin utiliza un enfoque de "instantánea" — lee los TODOs de las configuraciones, los renderiza como HTML de solo lectura y proporciona acciones basadas en la barra lateral para añadir elementos. El webview es una **capa de visualización**, no un formulario interactivo.
```javascript
'use strict';
