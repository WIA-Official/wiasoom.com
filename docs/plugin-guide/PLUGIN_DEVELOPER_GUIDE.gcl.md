...
When yuh `activate(context)` function get call, `context` (or `ctx`) provide dem APIs:
```typescript
...
|-----------|------|-------------|
| `sessionId` | `string` | Di terminal session to send to |
| `data` | `string` | Di command or data to send |
```javascript
...
| Parameter | Type | Description |
|-----------|------|-------------|
| `sessionId` | `string` | Di terminal session to watch |
...
unsubscribe();
```
**Important:** Always save di unsubscribe function an call it in `deactivate()` to prevent memory leaks.

---

### `ctx.sftp` — File transfer

> **Status: Coming Soon** — Di SFTP API is defined but not yet wired to di app's SFTP engine. `list()` currently returns an empty array, an `upload()`/`download()` are no-ops. Dis will be fully implemented in a future release. For now, use `ctx.terminal.send()` with `scp` or `rsync` commands as a workaround.
...
> **Compatibility note:** Some older plugins use positional arguments like `addSidebarButton(id, icon, label, onClick)`. Di official API uses an **options object** as documented above. Always use di object style for new plugins.

#### `ui.openWebview(options)`

Open a popup window with custom HTML content. Dis is how yuh build rich UIs.
...
Plugin settings dey store permanently in `~/.wia-soom/plugins/{your-plugin}/.plugin-settings.json`.
...
```
Returns `undefined` if the key don't exist.
...
> **Status: Coming Soon** — The AI API define but not yet connect to Soomy. Currently returns `{ response: 'AI not yet connected' }`. Full AI integration plan for a future release.
...
The `openWebview()` API let you build dashboard UIs with HTML, CSS, and JavaScript — all inside a popup window.

> **Important limitation:** Webviews are **display-only**. Dem cyah call back into plugin APIs (`ctx.settings`, `ctx.terminal`, etc.). Use sidebar buttons for all user actions, and use `openWebview()` to display current state. If you need interactive features, trigger dem from sidebar buttons and re-open the webview to refresh the display.

### Pattern: Terminal Command → Parse Output → Show in HTML

Dis is the most common plugin pattern. You run a command, parse the result, and display it visually.
```javascript
exports.activate = function(context) {
  context.ui.addSidebarButton({
    icon: 'hard-drive',
...
> **Note:** Webviews are display-only — dem cyah call back into plugin APIs. Use `ctx.settings` in your sidebar button handlers to modify settings, and use `openWebview()` to show the current state.
```javascript
