...
- [Partie 3 : Construction d'une Interface Utilisateur Personnalisée avec des Webviews](#partie-3-construction-dune-interface-utilisateur-personnalisée-avec-des-webviews)
...
S'abonner à toute sortie d'une session de terminal. Renvoie une **fonction de désabonnement**.
...
| `icon` | `string` | Oui | Nom de l'icône Lucide (ex. : `'server'`, `'shield'`, `'database'`) |
...
Ouvrir une fenêtre popup avec du contenu HTML personnalisé. C'est ainsi que vous construisez des interfaces riches.
...
L'API `openWebview()` vous permet de créer des interfaces de tableau de bord avec HTML, CSS et JavaScript — le tout dans une fenêtre popup.

> **Limitation importante :** Les webviews sont **uniquement d'affichage**. Elles ne peuvent pas rappeler les API des plugins (`ctx.settings`, `ctx.terminal`, etc.). Utilisez des boutons de barre latérale pour toutes les actions de l'utilisateur, et utilisez `openWebview()` pour afficher l'état actuel. Si vous avez besoin de fonctionnalités interactives, déclenchez-les à partir des boutons de la barre latérale et rouvrez la webview pour actualiser l'affichage.
...
| **NE JAMAIS** récupérer des URL externes | Exception : points de terminaison API de `wiasoom.com` |
...
Un plugin qui gère une liste de TODO en utilisant des paramètres pour un stockage persistant et une webview pour l'affichage.

> **Modèle de conception :** Étant donné que les webviews ne peuvent pas appeler directement les APIs des plugins, ce plugin utilise une approche de "snapshot" — il lit les TODO à partir des paramètres, les rend en HTML en lecture seule, et fournit des actions basées sur la barre latérale pour ajouter des él��ments. La webview est une **couche d'affichage**, pas un formulaire interactif.
```javascript
'use strict';
