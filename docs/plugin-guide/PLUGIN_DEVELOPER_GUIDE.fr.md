...
<p align="center">Créez des outils serveurs puissants, des tableaux de bord et des automatisations — directement dans WIA SOOM.</p>
...
- [Annexe : Catégories et icônes](#annexe-catégories--icônes)
...
Envoyer une commande (ou des données) à une session de terminal active.
...
| **Renvoie** | `() => void` | Appelez ceci pour arrêter l'écoute |
```javascript
...
```
**Référence des icônes :** Parcourez toutes les icônes disponibles sur [lucide.dev/icons](https://lucide.dev/icons)
...
Ouvrir une fenêtre contextuelle avec du contenu HTML personnalisé. C'est ainsi que vous construisez des interfaces utilisateur riches.
...
Ajouter un élément de texte persistant à la barre d'état inférieure.
...
Enregistrer une valeur. Prend en charge les chaînes, les nombres, les booléens, les tableaux et les objets.
```javascript
...
## Partie 3 : Construire une interface utilisateur personnalisée avec des webviews

L'API `openWebview()` vous permet de créer des interfaces de tableau de bord avec HTML, CSS et JavaScript — le tout dans une fenêtre pop-up.

> **Limitation importante :** Les webviews sont **uniquement en affichage**. Elles ne peuvent pas appeler les API des plugins (`ctx.settings`, `ctx.terminal`, etc.). Utilisez des boutons de barre latérale pour toutes les actions utilisateur, et utilisez `openWebview()` pour afficher l'état actuel. Si vous avez besoin de fonctionnalités interactives, déclenchez-les à partir des boutons de la barre latérale et rouvrez la webview pour actualiser l'affichage.

### Modèle : Commande Terminal → Analyser la Sortie → Afficher en HTML
...
- **Utilité** — est-ce que cela résout un vrai problème ?
...
| Règle | Pourquoi |
|------|-----|
| **NE JAMAIS** utiliser `eval()` ou `new Function()` | Risque d'injection de code |
| **NE JAMAIS** utiliser `child_process`, `exec()`, `spawn()` | Utilisez uniquement `ctx.terminal.send()` pour les commandes |
| **NE JAMAIS** récupérer des URLs externes | Exception : points de terminaison API `wiasoom.com` |
| **NE JAMAIS** accéder à `process.env` | Les variables d'environnement peuvent contenir des secrets |
| **NE JAMAIS** utiliser `require('fs')` directement | Utilisez `ctx.settings` pour le stockage, `ctx.sftp` pour le transfert de fichiers |
| **NE JAMAIS** utiliser des packages externes npm | JavaScript pur uniquement — pas de node_modules |
| **DOIT** utiliser `ctx.terminal.send()` pour toutes les commandes distantes | Cela passe par le canal SSH sécurisé |
| **DOIT** nettoyer dans `deactivate()` | Supprimez les écouteurs, effacez les intervalles |
...
Un plugin qui gère une liste TODO en utilisant les paramètres pour le stockage persistant et une webview pour l'affichage.
...
<p align="center"><em>Créez quelque chose d'incroyable. Partagez-le avec le monde.</em></p>
