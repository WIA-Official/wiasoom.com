...
- [Deel 2: Plugin Context API Referentie](#deel-2-plugin-context-api-referentie)
  ...
  - [ctx.settings](#ctxsettings--permanente-opslag)
...
- [Bijlage: Categorieën & Iconen](#bijlage-categorieën--iconen)
...
Abonneer je op alle output van een terminalsessie. Geeft een **afmeldfunctie** terug.

| Parameter | Type | Beschrijving |
|-----------|------|-------------|
| `sessionId` | `string` | De terminalsessie om te volgen |
| `callback` | `(data: string) => void` | Wordt aangeroepen met elke chunk van output |
| **Geeft terug** | `() => void` | Roep dit aan om te stoppen met luisteren |
```javascript
...
> **Status: Binnenkort Beschikbaar** — De SFTP API is gedefinieerd maar nog niet verbonden met de SFTP-engine van de app. `list()` geeft momenteel een lege array terug, en `upload()`/`download()` zijn no-ops. Dit zal volledig geïmplementeerd worden in een toekomstige release. Voor nu, gebruik `ctx.terminal.send()` met `scp` of `rsync` commando's als een workaround.
...
> **Opmerking:** Webviews zijn alleen voor weergave — ze kunnen geen terugroepacties doen naar plugin-API's. Gebruik `ctx.settings` in je zijbalkknophandlers om instellingen te wijzigen, en gebruik `openWebview()` om de huidige status weer te geven.
```javascript
...
Deze regels zijn **verplicht**. Plugins die deze schenden zullen worden afgewezen.
...
| **NOOIT** gebruik npm externe pakketten | Alleen pure JavaScript — geen node_modules |
| **MOET** `ctx.terminal.send()` gebruiken voor alle externe commando's | Dit gaat via het beveiligde SSH-kanaal |
...
> **Ontwerppatroon:** Aangezien webviews niet direct plugin-API's kunnen aanroepen, gebruikt deze plugin een "snapshot" benadering — het leest TODO's uit de instellingen, rendert ze als alleen-lezen HTML, en biedt sidebar-gebaseerde acties voor het toevoegen van items. De webview is een **weergave** laag, geen interactief formulier.
```javascript
'use strict';
...
Houdt de terminaluitvoer in de gaten en stuurt een melding wanneer specifieke patronen worden gedetecteerd.
```javascript
'use strict';
...
| `analytics` | Analyse en rapportage |
...
<p align="center"><em>Bouw iets geweldigs. Deel het met de wereld.</em></p>
