...
> **Nota:** Le webviews sono solo di visualizzazione — non possono richiamare le API del plugin. Usa `ctx.settings` nei gestori dei pulsanti della barra laterale per modificare le impostazioni e usa `openWebview()` per mostrare lo stato attuale.
§§§CHUNK_SEPARATOR§§§
---
