...
<p align="center"><strong>Build yuh own plugin in 5 minutes.</strong></p>
<p align="center">Create powerful server tools, dashboards, an automations — right inside WIA SOOM.</p>

---

## Table of Contents

- [Part 1: Quick Start — Yuh First Plugin in 5 Minutes](#part-1-quick-start--yuh-first-plugin-in-5-minutes)
...
- [Part 4: Publishing Yuh Plugin](#part-4-publishing-yuh-plugin)
...
## Part 1: Quick Start — Yuh First Plugin in 5 Minutes

### Wah yuh go build

A "Hello World" plugin dat add a button to di sidebar. When yuh click it, it show a notification.
...
Restart di app (or toggle di plugin off/on in Settings → Plugins).

Yuh should see a **"Hello World"** button in di sidebar. Click it — yuh go see a success notification!
...
When yuh `activate(context)` function get call, `context` (or `ctx`) provide dese APIs:
```typescript
...
|-----------|------|-------------|
| `sessionId` | `string` | Di terminal session fi send to |
| `data` | `string` | Di command or data fi send |
```javascript
...
| Parameter | Type | Description |
|-----------|------|-------------|
| `sessionId` | `string` | Di terminal session fi watch |
| `callback` | `(data: string) => void` | Call when each chunk of output come |
| **Returns** | `() => void` | Call dis fi stop listening |
```javascript
...
unsubscribe();
```
**Important:** Always save di unsubscribe function an call it in `deactivate()` fi prevent memory leaks.

---

### `ctx.sftp` — File transfer

> **Status: Coming Soon** — Di SFTP API define but not yet wired to di app's SFTP engine. `list()` currently returns an empty array, an `upload()`/`download()` a no-ops. Dis will fully implement in a future release. Fi now, use `ctx.terminal.send()` with `scp` or `rsync` commands as a workaround.
...
```
**Workaround (until SFTP API live):**
```javascript
...
> **Compatibility note:** Some older plugins use positional arguments like `addSidebarButton(id, icon, label, onClick)`. Di official API use an **options object** as documented above. Always use di object style fi new plugins.

#### `ui.openWebview(options)`

Open a popup window with custom HTML content. Dis a how yuh build rich UIs.

| Option | Type | Description |
|--------|------|-------------|
| `title` | `string` | Window title |
| `html` | `string` | Full HTML content fi render |
```javascript
context.ui.openWebview({
  title: 'Server Status',
  html: `
...
```
> See [Part 3](#part-3-building-custom-ui-with-webviews) fi advanced webview patterns.
...
Add a persistent text item to di bottom status bar.

| Parameter | Type | Description |
|-----------|------|-------------|
| `id` | `string` | Unique ID fi dis status item |
| `text` | `string` | Text fi display |
```javascript
...
Plugin settings dem stored permanently in `~/.wia-soom/plugins/{your-plugin}/.plugin-settings.json`.
...
```
Returns `undefined` if di key nuh exist.
...
> **Status: Coming Soon** — Di AI API defined but not yet connected to Soomy. Currently returns `{ response: 'AI not yet connected' }`. Full AI integration planned fi a future release.

#### `ai.chat(messages, options?)`

Send messages to di AI assistant (Soomy).
```javascript
...
Di `openWebview()` API let yuh build dashboard UIs with HTML, CSS, and JavaScript — all inside a popup window.

> **Important limitation:** Webviews are **display-only**. Dem cyan’t call back into plugin APIs (`ctx.settings`, `ctx.terminal`, etc.). Use sidebar buttons fi all user actions, and use `openWebview()` fi display current state. If yuh need interactive features, trigger dem from sidebar buttons and re-open di webview fi refresh di display.

### Pattern: Terminal Command → Parse Output → Show in HTML

Dis a di most common plugin pattern. Yuh run a command, parse di result, and display it visually.
```javascript
exports.activate = function(context) {
  context.ui.addSidebarButton({
    icon: 'hard-drive',
...
> **Note:** Webviews are display-only — dem cyan’t call back into plugin APIs. Use `ctx.settings` in yuh sidebar button handlers fi modify settings, and use `openWebview()` fi show di current state.
```javascript
...
1. Copy yuh plugin to `~/.wia-soom/plugins/{your-plugin}/`
2. Restart WIA SOOM
3. Verify it work: sidebar button appear, features work correctly
4. Test edge cases: wah happen if no terminal connected?

### Step 2: Prepare fi submission

Yuh plugin folder must contain:
```
your-plugin/
