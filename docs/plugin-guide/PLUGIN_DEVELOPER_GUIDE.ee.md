...
<p align="center"><strong>Yɔ plugin nɔvi le 5 minutes.</strong></p>
<p align="center">Mɛkpɔ server nɔvi, dashboards, kple automations — le WIA SOOM me.</p>
...
```
> See [Part 3](#part-3-building-custom-ui-with-webviews) meɖe ɖeɖeɖe webview patterns.
...
```
**Aɖeɖe `package.json` aɖe:**

| Aɖe | Ɖeɖe | Aɖeɖe |
|-------|-------------|---------|
| `name` | Aɖe aɖe kebab-case | `"my-awesome-plugin"` |
| `version` | Semantic version | `"1.0.0"` |
| `description` | Aɖeɖe aɖe ɖeɖe | `"Monitors nginx access logs in real-time"` |
| `author` | Woɖe aɖe | `"John Doe"` |
| `main` | Aɖe aɖe | `"index.js"` |

**Aɖe aɖeɖe:**

| Aɖe | Ɖeɖe |
|-------|-------------|
| `license` | Aɖe aɖe (MIT aɖeɖe) |
| `keywords` | Aɖe aɖe aɖeɖe aɖeɖe | 
| `soom.minVersion` | Aɖe WIA SOOM aɖe aɖe aɖe |

### Ʋe 3: Kɔ Plugin Registry

1. ****Package** your plugin as a ZIP file
2. **Add** wo plugin kɔ `plugins/{your-plugin-name}/`
3. **Submit** a Pull Request

### Ʋe 4: Nɔvi kple nɔvi

Mia nɔvi nyuie le nyuie aɖe:

- **Sɔgbɔ** — nɔvi aɖe aɖe (kɔ [Security Rules](#security-rules))
- **Nyui** — Ʋe aɖe? Ʋe aɖe nyuie?
- **Nɔvi** — Ʋe aɖe nyuie?

Mia nɔvi:
1. Wo plugin aɖe kɔ `registry.json`
2. A ZIP bundle aɖe le `dist/`
3. Wo plugin aɖe ɖe **Plugin Store** le nyuie WIA SOOM aɖe!
...
Aɖe aɖe yiye **nɔvi**. Plugins aɖe aɖe aɖe aɖe bɔbɔ.

| Aɖe | Ɖeɖe |
|------|-----|
| **NEVER** use `eval()` or `new Function()` | Code injection risk |
| **NEVER** use `child_process`, `exec()`, `spawn()` | Nɔvi `ctx.terminal.send()` le aɖe aɖe |
| **NEVER** fetch external URLs | Nɔvi: `wiasoom.com` API endpoints |
| **NEVER** access `process.env` | Environment variables aɖe aɖe aɖe |
| **NEVER** use `require('fs')` directly | Nɔvi `ctx.settings` le storage, `ctx.sftp` le file transfer |
| **NEVER** use npm external packages | Pure JavaScript nɔvi — nɔ node_modules |
| **MUST** use `ctx.terminal.send()` le nyuie aɖe aɖe | Aɖe aɖe le secure SSH channel |
| **MUST** clean up le `deactivate()` | Kɔ listeners, clear intervals |

### Error Handling

Mia nɔvi aɖe aɖe le try/catch:
```javascript
exports.activate = function(context) {
  try {
    // Your plugin logic
...
```
### Cleanup le deactivate()

Sɔ wo plugin aɖe kɔ intervals, listeners, kple subscriptions — kɔ aɖe:
```javascript
var intervals = [];
var unsubscribers = [];
...
WIA SOOM aɖe 254 languages. Le wo plugin label aɖe aɖe, nɔvi aɖe aɖe:
```javascript
...
## Ʋe 6: Nɔvi Aɖe le Eʋe

### Aɖe 1: Server Disk Checker

Runs `df -h` le remote server kple shows used/available space le status bar.
§§§CHUNK_SEPARATOR§§§
---

### Aɖe 2: TODO Manager

A plugin aɖe aɖe TODO list le settings le persistent storage kple a webview le display.

> **Design pattern:** Mɛ na webviews nɔvi aɖe aɖe plugin APIs, a plugin aɖe a "snapshot" approach — eɖe TODOs le settings, eɖe them as read-only HTML, kple eɖe sidebar-based actions le adding items. A webview aɖe a **display** layer, nɔvi an interactive form.
```javascript
'use strict';
...
Monitors terminal output kple sends a notification when specific patterns are detected.
```javascript
'use strict';
