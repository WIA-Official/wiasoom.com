...
- [Parte 1: Inizio Rapido — Il Tuo Primo Plugin in 5 Minuti](#parte-1-inizio-rapido--il-tuo-primo-plugin-in-5-minuti)
...
## Parte 1: Inizio Rapido — Il Tuo Primo Plugin in 5 Minuti
...
| Parametro | Tipo | Descrizione |
|-----------|------|-------------|
| `sessionId` | `string` | La sessione terminale da osservare |
| `callback` | `(data: string) => void` | Chiamato con ogni blocco di output |
| **Restituisce** | `() => void` | Chiamalo per smettere di ascoltare |
```javascript
...
> **Nota di compatibilità:** Alcuni plugin più vecchi usano argomenti posizionali come `addSidebarButton(id, icon, label, onClick)`. L'API ufficiale usa un **oggetto opzioni** come documentato sopra. Usa sempre lo stile oggetto per i nuovi plugin.

#### `ui.openWebview(options)`

Apri una finestra popup con contenuto HTML personalizzato. Questo è come costruisci interfacce utente ricche.
...
```
> Vedi [Parte 3](#part-3-building-custom-ui-with-webviews) par schemi avanzati de webview.
...
Aggiungi un elemento de testo persistente a la barra de stato in fondo.

| Parametro | Tipo | Descrizion |
|-----------|------|-------------|
| `id` | `string` | ID unico par sto elemento de stato |
| `text` | `string` | Testo da visualizzar |
```javascript
...
Le impostazioni del plugin le xe conservate permanentemente in `~/.wia-soom/plugins/{your-plugin}/.plugin-settings.json`.

#### `settings.get(key)`

Lèggi un valore salvato.
```javascript
var theme = context.settings.get('theme');       // 'dark'
var count = context.settings.get('refreshRate'); // 30
var items = context.settings.get('todoList');    // [{ text: '...', done: false }, ...]
```
Ritorna `undefined` se la chiave no la esiste.
...
> **Stato: In Arrivo** — L'API AI la xe definita ma no la xe ancora connessa a Soomy. Attualmente la ritorna `{ response: 'AI not yet connected' }`. L'integrazione completa de l'AI la xe prevista par una futura release.

#### `ai.chat(messages, options?)`

Invia messaggi al assistente AI (Soomy).
```javascript
...
L'API `openWebview()` te permette de costruir UI de dashboard con HTML, CSS e JavaScript — tuta dentro a na finestra popup.

> **Limitazione importante:** I webviews i xe **solo de visualizzazione**. No i può richiamar le API del plugin (`ctx.settings`, `ctx.terminal`, ecc.). Usa i bottoni de la sidebar par tutte le azioni de l'utente, e usa `openWebview()` par mostrar lo stato attuale. Se te gà bisogno de funzionalità interattive, attivale dai bottoni de la sidebar e riapri el webview par rinfrescar la visualizzazione.

### Schema: Comando del Terminale → Analizza l'Output → Mostra in HTML

Questo l'è el schema de plugin più comune. Te esegui un comando, analizzi el risultato e lo mostri visivamente.
```javascript
exports.activate = function(context) {
  context.ui.addSidebarButton({
    icon: 'hard-drive',
...
```
### Schema: Dashboard Interattivo con Auto-Rinfresco
```javascript
exports.activate = function(context) {
  context.ui.addSidebarButton({
    icon: 'activity',
...
```
### Schema: Visualizzazione delle Impostazioni in un Webview

> **Nota:** I webviews i xe solo de visualizzazione — no i può richiamar le API del plugin. Usa `ctx.settings` nei tuoi gestori de bottoni de la sidebar par modificare le impostazioni, e usa `openWebview()` par mostrar lo stato attuale.
```javascript
...
## Parte 4: Pubblicare el Tuo Plugin

### Passo 1: Testa localmente

1. Copia el tuo plugin in `~/.wia-soom/plugins/{your-plugin}/`
2. Riavvia WIA SOOM
3. Verifica che funzioni: el bottone de la sidebar el compare, le funzionalità le funziona correttamente
4. Testa i casi limite: che succede se no ghe xe nessun terminal connesso?

### Passo 2: Prepara par la sottomissione

La cartella del tuo plugin la deve contener:
```
your-plugin/
...
| `description` | Descrizione in una riga | `"Monitora i log di accesso di nginx in tempo reale"` |
...
| Campo | Descrizione |
|-------|-------------|
| `license` | Tipo di licenza (MIT consigliato) |
...
3. Il tuo plugin appare nel **Plugin Store** per tutti gli utenti WIA SOOM!
...
| **DEVE** pulire in `deactivate()` | Rimuovi i listener, cancella gli intervalli |
...
Esegue `df -h` sul server remoto e mostra lo spazio usato/disponibile nella barra di stato.
```javascript
'use strict';
...
Un plugin che gestisce una lista TODO utilizzando impostazioni per lo storage persistente e una webview per la visualizzazione.

> **Modello di design:** Poiché le webview non possono chiamare direttamente le API dei plugin, questo plugin utilizza un approccio "snapshot" — legge i TODO dalle impostazioni, li rende come HTML di sola lettura e fornisce azioni basate sulla sidebar per aggiungere elementi. La webview è uno strato di **visualizzazione**, non un modulo interattivo.
```javascript
'use strict';
...
### Esempio 3: Monitor degli Errori
...
| `kpop` | Strumenti legati al K-pop |
...
<p align="center"><em>Crea qualcosa di straordinario. Condividilo con il mondo.</em></p>
