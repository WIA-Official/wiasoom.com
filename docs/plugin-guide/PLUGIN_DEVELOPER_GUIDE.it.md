...
<p align="center"><strong>Costruisci il tuo plugin in 5 minuti.</strong></p>
...
Apri una finestra popup con contenuto HTML personalizzato. Questo è il modo in cui costruisci interfacce utente ricche.
...
> **Stato: In Arrivo** — L'API AI è definita ma non ancora collegata a Soomy. Attualmente restituisce `{ response: 'AI not yet connected' }`. È prevista un'integrazione completa dell'AI per un rilascio futuro.
...
| **MAI** usare pacchetti npm esterni | Solo JavaScript puro — niente node_modules |
...
Se il tuo plugin crea intervalli, listener o sottoscrizioni — puliscili:
```javascript
context.ui.openWebview({
  title: 'Server Status',
  html: `
...
Un plugin che gestisce una lista TODO utilizzando le impostazioni per lo storage persistente e una webview per la visualizzazione.

> **Modello di design:** Poiché le webview non possono chiamare direttamente le API del plugin, questo plugin utilizza un approccio "snapshot" — legge i TODO dalle impostazioni, li rende come HTML di sola lettura e fornisce azioni basate sulla barra laterale per aggiungere elementi. La webview è uno strato di **visualizzazione**, non un modulo interattivo.
```javascript
