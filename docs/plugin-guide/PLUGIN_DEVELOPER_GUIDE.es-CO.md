...
<p align="center">Crea herramientas de servidor poderosas, paneles de control y automatizaciones — directamente dentro de WIA SOOM.</p>
...
§§§CHUNK_SEPARATOR§§§
**Referencia de íconos:** Explora todos los íconos disponibles en [lucide.dev/icons](https://lucide.dev/icons)

> **Nota de compatibilidad:** Algunos plugins más antiguos usan argumentos posicionales como `addSidebarButton(id, icon, label, onClick)`. La API oficial utiliza un **objeto de opciones** como se documenta arriba. Siempre usa el estilo de objeto para nuevos plugins.
...
```
> Consulta [Parte 3](#part-3-building-custom-ui-with-webviews) para patrones avanzados de webview.
...
> **Nota:** Los webviews son solo de visualización — no pueden llamar a las APIs del plugin. Usa `ctx.settings` en los manejadores de botones de la barra lateral para modificar configuraciones, y utiliza `openWebview()` para mostrar el estado actual.
```javascript
...
| **NUNCA** obtener URLs externas | Excepción: endpoints de API de `wiasoom.com` |
...
Un plugin que gestiona una lista de TODO utilizando configuraciones para almacenamiento persistente y una webview para la visualización.

> **Patrón de diseño:** Dado que las webviews no pueden llamar directamente a las APIs de los plugins, este plugin utiliza un enfoque de "snapshot" — lee los TODOs de las configuraciones, los renderiza como HTML de solo lectura y proporciona acciones basadas en la barra lateral para agregar elementos. La webview es una **capa de visualización**, no un formulario interactivo.
```javascript
...
## Apéndice: Categorías e Íconos

### Categorías de Plugins (29)

Utiliza estos en tus `package.json` `keywords` o al enviar a el registro:
...
<p align="center"><em>Crea algo asombroso. Compártelo con el mundo.</em></p>
