...
<p align="center"><strong>5分钟内构建自己的插件。</strong></p>
...
- [第 1 部分：快速入门 — 5分钟内创建第一个插件](#part-1-quick-start--your-first-plugin-in-5-minutes)
...
- [第 4 部分：发布你的插件](#part-4-publishing-your-plugin)
- [第 5 部分：最佳实践](#part-5-best-practices)
- [第 6 部分：真实世界示例](#part-6-real-world-examples)
- [附录：类别与图标](#appendix-categories--icons)

---

## 第 1 部分：快速入门 — 5分钟内创建第一个插件
...
你应该在侧边栏看到一个 **"Hello World"** 按钮。点击它 — 你会看到一个成功通知！
...
将命令（或任何数据）发送到一个活动的终端会话。

| 参数 | 类型 | 描述 |
|-----------|------|-------------|
| `sessionId` | `string` | 要发送到的终端会话 |
| `data` | `string` | ���发送的命令或数据 |
§§§CHUNK_SEPARATOR§§§
...
| `callback` | `(data: string) => void` | 每次输出块调用 |
| **返回** | `() => void` | 调用此函数以停止监听 |
§§§CHUNK_SEPARATOR§§§
**重要：** 始终保存取消订阅函数，并在 `deactivate()` 中调用以防止内存泄漏。

---

### `ctx.sftp` — 文件传输

> **状态：即将推出** — SFTP API 已定义，但尚未连接到应用程序的 SFTP 引擎。`list()` 目前返回一个空数组，`upload()`/`download()` 是无操作。这将在未来的版本中完全实现。现在，使用 `ctx.terminal.send()` 结合 `scp` 或 `rsync` 命令作为变通方法。
...
> **兼容性说明：** 一些旧插件使用位置参数，如 `addSidebarButton(id, icon, label, onClick)`。官方 API 使用一个 **选项对象**，如上所述。对于新插件，请始终使用对象样式。
...
`openWebview()` API 允许您使用 HTML、CSS 和 JavaScript 构建仪表板 UI — 所有内容都在弹出窗口中。

> **重要限制：** Webviews 是 **仅用于显示**。它们不能回调插件 API（`ctx.settings`、`ctx.terminal` 等）。使用侧边栏按钮进行所有用户操作，并使用 `openWebview()` 显示当前状态。如果需要交互功能，请从侧边栏按钮触发它们，并重新打开 webview 以刷新显示。
...
> **注意：** Webviews 仅用于显示 — 它们不能回调插件 API。在您的侧边栏按钮处理程序中使用 `ctx.settings` 来修改设置，并使��� `openWebview()` 显示当前状态。
```javascript
...
2. **Add** 你的插件到 `plugins/{your-plugin-name}/`
3. **Submit** 一个 Pull Request

### 第 4 步：审核和批准

我们会审核每个插件以确保：

- **安全性** — 没有危险的 API（参见 [安全规则](#security-rules)）
- **质量** — 它是否有效？代码是否干净？
- **实用性** �� 它是否解决了实际问题？

审核通过后：
1. 你的插件将被添加到 `registry.json`
2. 在 `dist/` 中创建一个 ZIP 包
3. 你的插件将出现在所有 WIA SOOM 用户的 **插件商店** 中！
...
WIA SOOM 支持 254 种语言。要使你的插件标签可翻译，使用简单的方法：
```javascript
...
> **设计模式：** 由于 webviews 不能直接调用插件 API，这个插件使用了“快照”方法 — 它从设置中读取 TODO，将其呈现为只读 HTML，并提供基于侧边栏的操作以添加项目。webview 是一个 **显示** 层，而不是交互表单。
```javascript
...
在你的 `package.json` `keywords` 中使用这些，或在提交到注册表时使用：
