...
Restart aplikasi (atau toggle plugin mati/hidup di Pengaturan → Plugin).

Anda harus melihat tombol **"Hello World"** di sidebar. Klik tombol tersebut — Anda akan melihat notifikasi sukses!
...
> **Status: Segera Hadir** — API SFTP telah didefinisikan tetapi belum terhubung ke mesin SFTP aplikasi. `list()` saat ini mengembalikan array kosong, dan `upload()`/`download()` tidak melakukan apa-apa. Ini akan sepenuhnya diimplementasikan dalam rilis mendatang. Untuk saat ini, gunakan `ctx.terminal.send()` dengan perintah `scp` atau `rsync` sebagai solusi sementara.
...
```
**Referensi ikon:** Telusuri semua ikon yang tersedia di [lucide.dev/icons](https://lucide.dev/icons)
...
Buka jendela popup dengan konten HTML kustom. Ini adalah cara Anda membangun UI yang kaya.
...
```
> Lihat [Bagian 3](#part-3-building-custom-ui-with-webviews) untuk pola webview yang lebih lanjut.
...
Tambahkan item teks yang persisten ke bilah status bawah.
...
### `ctx.settings` — Penyimpanan persisten

Pengaturan plugin disimpan secara permanen di `~/.wia-soom/plugins/{your-plugin}/.plugin-settings.json`.
...
```
Mengembalikan `undefined` jika kunci tidak ada.

#### `settings.set(key, value)`

Simpan nilai. Mendukung string, angka, boolean, array, dan objek.
```javascript
...
```
**Contoh: Ingat preferensi pengguna**
```javascript
...
> **Status: Segera Hadir** — API AI telah didefinisikan tetapi belum terhubung ke Soomy. Saat ini mengembalikan `{ response: 'AI not yet connected' }`. Integrasi AI penuh direncanakan untuk rilis mendatang.

#### `ai.chat(messages, options?)`

Kirim pesan ke asisten AI (Soomy).
```javascript
...
## Bagian 3: Membangun UI Kustom dengan Webviews

API `openWebview()` memungkinkan Anda membangun UI dasbor dengan HTML, CSS, dan JavaScript — semuanya di dalam jendela popup.

> **Batasan penting:** Webviews adalah **hanya tampilan**. Mereka tidak dapat memanggil kembali ke API plugin (`ctx.settings`, `ctx.terminal`, dll.). Gunakan tombol sidebar untuk semua tindakan pengguna, dan gunakan `openWebview()` untuk menampilkan status saat ini. Jika Anda memerlukan fitur interaktif, aktifkan dari tombol sidebar dan buka kembali webview untuk menyegarkan tampilan.

### Pola: Perintah Terminal → Parse Output → Tampilkan dalam HTML

Ini adalah pola plugin yang paling umum. Anda menjalankan perintah, mem-parsing hasilnya, dan menampilkannya secara visual.
```javascript
exports.activate = function(context) {
  context.ui.addSidebarButton({
    icon: 'hard-drive',
...
```
### Pola: Dasbor Interaktif dengan Penyegaran Otomatis
```javascript
exports.activate = function(context) {
  context.ui.addSidebarButton({
    icon: 'activity',
...
```
### Pola: Menampilkan Pengaturan dalam Webview

> **Catatan:** Webviews hanya untuk tampilan — mereka tidak dapat memanggil kembali ke API plugin. Gunakan `ctx.settings` dalam pengendali tombol sidebar Anda untuk memodifikasi pengaturan, dan gunakan `openWebview()` untuk menunjukkan status saat ini.
```javascript
...
## Bagian 4: Menerbitkan Plugin Anda

### Langkah 1: Uji secara lokal

1. Salin plugin Anda ke `~/.wia-soom/plugins/{your-plugin}/`
2. Restart WIA SOOM
3. Verifikasi bahwa itu berfungsi: tombol sidebar muncul, fitur berfungsi dengan benar
4. Uji kasus tepi: apa yang terjadi jika tidak ada terminal yang terhubung?

### Langkah 2: Siapkan untuk pengajuan

Folder plugin Anda harus berisi:
```
your-plugin/
├── package.json    ← manifest (required fields below)
└── index.js        ← must export activate(context)
```
**Field `package.json` yang Diperlukan:**

| Field | Deskripsi | Contoh |
|-------|-------------|---------|
| `name` | ID unik dalam kebab-case | `"my-awesome-plugin"` |
| `version` | Versi semantik | `"1.0.0"` |
| `description` | Deskripsi satu baris | `"Memantau log akses nginx secara real-time"` |
...
| Field | Deskripsi |
|-------|-------------|
| `license` | Tipe lisensi (MIT disarankan) |
| `keywords` | Array tag pencarian |
| `soom.minVersion` | Versi WIA SOOM minimum yang dibutuhkan |

### Langkah 3: Kirim ke Plugin Registry

1. ****Package** your plugin as a ZIP file
2. **Tambahkan** plugin Anda ke `plugins/{your-plugin-name}/`
3. **Kirim** Pull Request

### Langkah 4: Tinjau dan Persetujuan

Kami meninjau setiap plugin untuk:

- **Keamanan** — tidak ada API berbahaya (lihat [Aturan Keamanan](#security-rules))
- **Kualitas** — apakah berfungsi? Apakah kodenya bersih?
- **Kegunaan** — apakah menyelesaikan masalah nyata?
...
Memantau output terminal dan mengirimkan notifikasi ketika pola tertentu terdeteksi.
```javascript
'use strict';
...
Gunakan ini di `package.json` `keywords` atau saat mengirim ke registry:
...
<p align="center"><em>Buat sesuatu yang luar biasa. Bagikan dengan dunia.</em></p>
