...
<p align="center"><strong>Ng diak a kirel a plugin el 5 minutes.</strong></p>
<p align="center">Mekngii a server tools, dashboards, e a automations — el kirel a WIA SOOM.</p>
...
A "Hello World" plugin a kirel a button el sidebar. Ngei er a kirel, a ngii a notification.
...
Restart a app (o a toggle a plugin off/on el Settings → Plugins).

A diak a ngii a **"Hello World"** button el sidebar. Click a — a ngii a success notification!
...
Ngei a `activate(context)` function a kirel, a `context` (o `ctx`) a ngii a APIs:
§§§CHUNK_SEPARATOR§§§
---
...
Send a command (o a data) el a active terminal session.

| Parameter | Type | Description |
|-----------|------|-------------|
| `sessionId` | `string` | A terminal session a send a |
| `data` | `string` | A command o a data a send a |
```typescript
...
Subscribe el a output a a terminal session. Returns a **unsubscribe function**.

| Parameter | Type | Description |
|-----------|------|-------------|
| `sessionId` | `string` | A terminal session a watch a |
| `callback` | `(data: string) => void` | A kirel el a chunk a output |
| **Returns** | `() => void` | A kirel a stop a listening |
```javascript
// Send a command to the terminal
// Don't forget the \n (newline) to execute it!
context.terminal.send(sessionId, 'df -h\n');
```
**Important:** A diak a save a unsubscribe function e a kirel el `deactivate()` a prevent a memory leaks.

---

### `ctx.sftp` — File transfer

> **Status: Coming Soon** — A SFTP API a kirel a defined pero a diak a wired el a app's SFTP engine. `list()` a kirel a return a empty array, e `upload()`/`download()` a kirel a no-ops. A diak a fully implemented el a future release. El a diak, a use `ctx.terminal.send()` el a `scp` o `rsync` commands a workaround.

#### `sftp.list(sessionId, path)`

List a files el a remote directory.
```javascript
...
Upload a file el a local machine el a remote server.
```javascript
...
Download a file el a remote server el a local machine.
```javascript
...
Add a button el a WIA SOOM sidebar.

| Option | Type | Required | Description |
|--------|------|----------|-------------|
| `id` | `string` | No | Unique ID (defaults to plugin name) |
| `icon` | `string` | Yes | Lucide icon name (e.g., `'server'`, `'shield'`, `'database'`) |
| `label` | `string` | Yes | Button text a ngii el sidebar |
| `onClick` | `() => void` | Yes | Function a kirel el a button a kirel |
```javascript
// Use terminal commands instead
context.terminal.send(sessionId, 'scp user@host:/var/log/app.log /tmp/\n');
```
**Icon reference:** Browse all available icons el [lucide.dev/icons](https://lucide.dev/icons)

> **Compatibility note:** A diak a use a positional arguments el a older plugins a `addSidebarButton(id, icon, label, onClick)`. A official API a kirel a use a **options object** a documented el a above. A diak a use a object style el a new plugins.

#### `ui.openWebview(options)`

Open a popup window el a custom HTML content. A diak a build a rich UIs.

| Option | Type | Description |
|--------|------|-------------|
| `title` | `string` | Window title |
| `html` | `string` | Full HTML content a render a |
```javascript
context.ui.addSidebarButton({
  id: 'my-dashboard',
...
|----------|-------------|
| `server` | Ngir a server a klung |
| `devtools` | Ngir a klung a chelsel |
| `calculator` | Ngir a calculator a klung |
| `simulator` | Ngir a simulator a klung |
| `game` | Ngir a game a klung |
| `business` | Ngir a business a klung |
| `security` | Ngir a security a klung |
| `web` | Ngir a web server a klung |
| `education` | Ngir a education a klung |
| `health` | Ngir a health a klung |
| `islamic` | Ngir a Islamic a klung (ngir a prayei, etc.) |
| `science` | Ngir a science a klung |
| `quantum` | Ngir a quantum a klung |
| `ai` | Ngir a AI a klung |
| `biotech` | Ngir a biotechnology a klung |
| `space` | Ngir a space a klung |
| `network` | Ngir a network a klung |
| `database` | Ngir a database a klung |
| `monitoring` | Ngir a monitoring a klung |
| `devops` | Ngir a DevOps a klung |
| `utility` | Ngir a utility a klung |
| `design` | Ngir a design a klung |
| `ecommerce` | Ngir a e-commerce a klung |
| `automation` | Ngir a automation a klung |
| `kpop` | Ngir a K-pop a klung |
| `accessibility` | Ngir a accessibility a klung |
| `analytics` | Ngir a analytics a klung |
| `wia` | Ngir a WIA a klung |
| `all` | Ngir a klung a ngii a klung |
...
|-----------|---------|
| `server` | Ngir a server a klung |
| `shield` | Ngir a security |
| `database` | Ngir a database |
| `activity` | Ngir a monitoring |
| `terminal` | Ngir a terminal a klung |
| `code` | Ngir a development |
| `hard-drive` | Ngir a disk/storage |
| `network` | Ngir a networking |
| `lock` | Ngir a auth/encryption |
| `eye` | Ngir a watching/monitoring |
| `check-square` | Ngir a tasks/TODO |
| `layout-dashboard` | Ngir a dashboards |
| `settings` | Ngir a configuration |
| `zap` | Ngir a automation |
| `globe` | Ngir a web/international |
