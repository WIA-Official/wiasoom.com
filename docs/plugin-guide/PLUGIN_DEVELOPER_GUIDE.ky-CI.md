...
A "Hello World" plugin dat adds a button to di sidebar. When clicked, it shows a notification.
...
Yuh should see a **"Hello World"** button in di sidebar. Click it — yuh'll see a success notification!
...
When yuh `activate(context)` function is called, `context` (or `ctx`) provides dese APIs:
```typescript
...
unsubscribe();
```
**Important:** Always save di unsubscribe function and call it in `deactivate()` to prevent memory leaks.

---

### `ctx.sftp` — File transfer

> **Status: Coming Soon** — Di SFTP API is defined but not yet wired to di app's SFTP engine. `list()` currently returns an empty array, and `upload()`/`download()` are no-ops. Dis will be fully implemented in a future release. For now, use `ctx.terminal.send()` with `scp` or `rsync` commands as a workaround.
