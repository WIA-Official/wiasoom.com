...
> **Nota:** Los webviews son solo de visualización — no pueden llamar de vuelta a las APIs del plugin. Usa `ctx.settings` en los controladores de botones de tu barra lateral para modificar configuraciones, y usa `openWebview()` para mostrar el estado actual.
```javascript
...
| **NUNCA** obtengas URLs externas | Excepción: endpoints de API de `wiasoom.com` |
