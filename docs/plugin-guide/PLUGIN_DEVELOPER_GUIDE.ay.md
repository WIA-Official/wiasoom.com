...
- [Parte 2: Referencia de API del Contexto del Plugin](#parte-2-referencia-de-api-del-contexto-del-plugin)
...
Un plugin "Hola Mundo" que añade un botón a la barra lateral. Cuando se hace clic, muestra una notificación.
...
Reinicia la aplicación (o activa/desactiva el plugin en Configuraciones → Plugins).

Deberías ver un **"Hola Mundo"** botón en la barra lateral. ¡Haz clic en él — verás una notificación de éxito!
...
## Parte 2: Referencia de API del Contexto del Plugin
...
Suscríbete a toda la salida de una sesión de terminal. Devuelve una **función de desuscripción**.
...
unsubscribe();
```
**Importante:** Siempre guarda la función de desuscripción y llámala en `deactivate()` para prevenir fugas de memoria.
...
| `icon` | `string` | Sí | Nombre del ícono de Lucide (por ejemplo, `'server'`, `'shield'`, `'database'`) |
...
```
> Vean [Parte 3](#part-3-building-custom-ui-with-webviews) para patrones avanzados de webview.
...
Agrega un elemento de texto persistente a la barra de estado inferior.
...
La configuración del plugin se almacena permanentemente en `~/.wia-soom/plugins/{your-plugin}/.plugin-settings.json`.
...
```
### Patr��n: Panel de Control Interactivo con Actualización Automática
```javascript
exports.activate = function(context) {
  context.ui.addSidebarButton({
    icon: 'activity',
...
> **Nota:** Los webviews son solo para visualización — no pueden llamar a las APIs de plugin. Usa `ctx.settings` en los manejadores de botones de tu barra lateral para modificar configuraciones, y usa `openWebview()` para mostrar el estado actual.
```javascript
...
4. Prueba casos extremos: ¿qué sucede si no hay terminal conectada?
...
- **Calidad** — ¿funciona? ¿Es el código limpio?
...
| **NUNCA** obtenga URLs externas | Excepción: puntos finales de la API de `wiasoom.com` |
...
|----------|-------------|
| `server` | Server jach'a qamañataki |
| `devtools` | Desarrollo jach'a qamañataki |
| `calculator` | Calculadoras ukat convertidores |
| `simulator` | Simuladores |
| `game` | Terminal jach'a qamañataki |
| `business` | Empresarial jach'a qamañataki |
| `security` | Seguridad ukat auditoría |
| `web` | Web server jach'a qamañataki |
| `education` | Educación jach'a qamañataki |
| `health` | Salud ukat jach'a qamañataki |
| `islamic` | Islámico jach'a qamañataki (salaqani tiempo, etc.) |
| `science` | Científico jach'a qamañataki |
| `quantum` | Quantum computing jach'a qamañataki |
| `ai` | AI-powered jach'a qamañataki |
| `biotech` | Biotecnología jach'a qamañataki |
| `space` | Espacio ukat astronomía jach'a qamañataki |
| `network` | Red jach'a qamañataki |
| `database` | Base de datos jach'a qamañataki |
| `monitoring` | Server monitorización |
| `devops` | DevOps ukat CI/CD |
| `utility` | Jach'a utilidad |
| `design` | Diseño jach'a qamañataki |
| `ecommerce` | E-commerce jach'a qamañataki |
| `automation` | Automatización jach'a qamañataki |
| `kpop` | K-pop ukat jach'a qamañataki |
| `accessibility` | Accesibilidad jach'a qamañataki |
| `analytics` | Análisis ukat reporte |
| `wia` | WIA ecosistema jach'a qamañataki |
| `all` | Todas categorías ukat jach'a qamañataki |
...
| `terminal` | Terminal jach'a qamañataki |
| `code` | Desarrollo |
| `hard-drive` | Disco/almacenamiento |
| `network` | Red |
| `lock` | Autenticación/encriptación |
| `eye` | Observación/monitorización |
