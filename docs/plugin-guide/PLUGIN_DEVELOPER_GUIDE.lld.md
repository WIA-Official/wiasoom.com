...
<p align="center">Crea potenti strumenti server, dashboard e automazioni — direttamente dentro WIA SOOM.</p>
...
```
**Riferimento icone:** Scorri tutte le icone disponibili su [lucide.dev/icons](https://lucide.dev/icons)
...
```
> Vëd [Part 3](#part-3-building-custom-ui-with-webviews) për schemi avansé de webview.

#### `ui.showNotification(type, message)`

Mostra na notifica de toast.

| Parameter | Type | Description |
|-----------|------|-------------|
| `type` | `'success' \| 'error' \| 'info'` | Stile de notifica |
| `message` | `string` | Testo da mostra |
```javascript
...
Agiungi n'element de testo persistente a la barra de stato in fondo.

| Parameter | Type | Description |
|-----------|------|-------------|
| `id` | `string` | ID unich per chëst element de stato |
| `text` | `string` | Testo da visualizé |
```javascript
...
I settaggi del plugin i è stoccati permanentement in `~/.wia-soom/plugins/{your-plugin}/.plugin-settings.json`.
...
```
Rëturna `undefined` se l'key non esiste.

#### `settings.set(key, value)`

Salva n'valore. Supporta stringhe, numeri, booleani, array e oggetti.
```javascript
...
> **Status: In Arrivo** — L'API AI è definita ma non è ancora connessa a Soomy. Attualmente rëturna `{ response: 'AI not yet connected' }`. L'integrazione completa dell'AI è pianificata per una futura release.
...
## Part 3: Costruire UI Personalizzate con Webviews

L'API `openWebview()` te permette de costruire UI de dashboard con HTML, CSS e JavaScript — tüt dené n'fenestra popup.

> **Limitazione Importante:** I webviews i è **solo de visualizazion**. I non ponno richiamé le API del plugin (`ctx.settings`, `ctx.terminal`, ecc.). Usa i bottoni de sidebar per tüt i azioni dell'utente, e usa `openWebview()` per mostrare lo stato attuale. Se te serve funzioni interattive, attivale dai bottoni de sidebar e riapri il webview per rinfrescà la visualizazion.

### Schema: Comando del Terminale → Analizza l'Output → Mostra in HTML

Chësta è la schema de plugin la più comune. Te esegui un comando, analizzi il risultato e lo mostri visivamente.
```javascript
exports.activate = function(context) {
  context.ui.addSidebarButton({
    icon: 'hard-drive',
...
```
### Schema: Dashboard Interattiva con Auto-Rinfresco
```javascript
exports.activate = function(context) {
  context.ui.addSidebarButton({
    icon: 'activity',
...
```
### Schema: Visualizzazione dei Settaggi in un Webview

> **Nota:** I webviews i è solo de visualizazion — i non ponno richiamé le API del plugin. Usa `ctx.settings` nei tuoi gestori de bottoni de sidebar per modificà i settaggi, e usa `openWebview()` per mostrare lo stato attuale.
```javascript
...
## Part 4: Pubblicare il Tuo Plugin

### Passo 1: Testa localmente

1. Copia il tuo plugin in `~/.wia-soom/plugins/{your-plugin}/`
2. Riavvia WIA SOOM
3. Verifica che funzioni: il bottone de sidebar appare, le funzioni funzionano correttamente
4. Testa i casi limite: che succede se non è connesso nessun terminale?

### Passo 2: Prepara per la sottomissione

La cartella del tuo plugin deve contenere:
```
your-plugin/
...
Queste regole sono **obbligatorie**. I plugin che le violano saranno respinti.
...
| **MAI** usare `require('fs')` direttamente | Usa `ctx.settings` per la memorizzazione, `ctx.sftp` per il trasferimento di file |
...
### Esempio 1: Controllore Disco del Server
...
Un plugin che gestisce una lista TODO utilizzando impostazioni per la memorizzazione persistente e un webview per la visualizzazione.

> **Pattern di design:** Poiché i webview non possono chiamare direttamente le API del plugin, questo plugin utilizza un approccio "snapshot" — legge i TODO dalle impostazioni, li rende come HTML di sola lettura e fornisce azioni basate sulla barra laterale per aggiungere elementi. Il webview è uno strato di **visualizzazione**, non un modulo interattivo.
```javascript
'use strict';
...
Usa questi nei tuoi `package.json` `keywords` o quando invii al registro:
...
| `business` | Strumenti per le aziende |
