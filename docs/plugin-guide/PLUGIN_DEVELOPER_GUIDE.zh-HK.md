...
- [第一部分：快速入門 — 你的第一個插件在5分鐘內](#part-1-quick-start--your-first-plugin-in-5-minutes)
...
- [第三部分：使用 Webviews 構建自定義 UI](#part-3-building-custom-ui-with-webviews)
- [第四部分：發布你的插件](#part-4-publishing-your-plugin)
- [第五部分：最佳實踐](#part-5-best-practices)
- [第六部分：實際範例](#part-6-real-world-examples)
- [附錄：類別與圖標](#appendix-categories--icons)

---

## 第一部分：快速入門 — 你的第一個插件在5分鐘內

### 你將構建的內容

一個「Hello World」插件，將一個按鈕添加到側邊欄。點擊時，它會顯示一個通知。
...
重啟應用程式（或在設置 → 插件中切換插件的開/關）。

你應該在側邊欄看到一個 **"Hello World"** 按鈕。點擊它 — 你會看到一個成功通知！
...
> **狀態：即將推出** — SFTP API 已定義但尚未連接到應用的 SFTP 引擎。`list()` 目前返回一個空數組，`upload()`/`download()` 目前無操作。這將在未來的版本中完全實現。現在，請使用 `ctx.terminal.send()` 搭配 `scp` 或 `rsync` 命令作為變通方法。
...
打開一個帶有自定義 HTML 內容的彈出窗口。這是構建豐富 UI 的方式。
...
> **重要限制：** Webviews 是 **僅顯示** 的。它們不能回調插件 API（`ctx.settings`、`ctx.terminal` 等）。對於所有用戶操作，請使用側邊按鈕，並使用 `openWebview()` 顯示當前狀態。如果需要互動功能，請從側邊按鈕觸發它們，並重新打開 webview 以刷新顯示。
...
> **注意：** Webviews 是僅顯示的 — 它們不能回調插件 API。請在你的側邊按鈕處理程序中使用 `ctx.settings` 來修改設置，並使用 `openWebview()` 來顯示當前狀態。
```javascript
...
批准後：
1. 你的插件會被添加�� `registry.json`
2. 在 `dist/` 中創建 ZIP 包
3. 你的插件將出現在所有 WIA SOOM 用戶的 **插件商店** 中！
...
WIA SOOM 支持 254 種語言。要使你的插件標籤可翻譯，請使用簡單的方法：
```javascript
...
一個使用設置進行持久存儲並使用 webview 進行顯示的 TODO 列表管理插件。

> **設計模式：** 由於 webviews 不能直接調用插件 API，這個插件使用了 "快照" 方法 — 它從設置中讀取 TODO，將它們渲染為只讀 HTML，並提供基於側邊欄的操作來添加項目。webview 是一個 **顯示** 層，而不是一個交互式表單。
```javascript
...
在你的 `package.json` 的 `keywords` 中或提交到註冊表時使用這些類別：
...
<p align="center"><em>創造一些驚人的東西。與世界分享。</em></p>
