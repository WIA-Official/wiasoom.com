...
```
> Mira [Parte 3](#part-3-building-custom-ui-with-webviews) para patrones avanzados de webview.
...
> **Nota:** Los webviews son solo para visualización — no pueden llamar de vuelta a las APIs de plugins. Usa `ctx.settings` en tus manejadores de botones de la barra lateral para modificar configuraciones, y usa `openWebview()` para mostrar el estado actual.
```javascript
...
| **NUNCA** obtenga URLs externas | Excepción: endpoints de API de `wiasoom.com` |
...
Un plugin que gestiona una lista de TODO usando configuraciones para almacenamiento persistente y un webview para visualización.

> **Patrón de diseño:** Dado que los webviews no pueden llamar directamente a las APIs de plugins, este plugin utiliza un enfoque de "instantánea" — lee los TODOs de las configuraciones, los renderiza como HTML de solo lectura y proporciona acciones basadas en la barra lateral para añadir elementos. El webview es una **capa de visualización**, no un formulario interactivo.
```javascript
'use strict';
...
## Apéndice: Kategorías & Íconos

### Kategorías de Plugins (29)

Usa estos en tu `package.json` `keywords` o al enviar a la registración:
