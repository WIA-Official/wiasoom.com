...
<p align="center"><strong>Rakenda oma plugin 5 minutis.</strong></p>
...
- [Osa 2: Plugin Context API Viide](#osa-2-plugin-context-api-viide)
  - [ctx.terminal](#ctxterminal--käivita-käsklusi-kaugreserverites)
...
Taaskäivita rakendus (või lülita plugin seadetes välja/ sisse).

Sa peaksid nägema **"Hello World"** nuppu külgribal. Kliki sellele — sa näed eduka teate!
...
### `ctx.terminal` — Käivita käsklusi kaugreserverites
...
Telli kõik väljundid terminali sessioonist. Tagastab **tühistamise funktsiooni**.

| Parameeter | Tüüp | Kirjeldus |
|------------|------|-----------|
| `sessionId` | `string` | Terminali sessioon, mida jälgida |
| `callback` | `(data: string) => void` | Kutsutakse iga väljunditükiga |
| **Tagastab** | `() => void` | Kutsu seda, et lõpetada kuulamine |
§§§CHUNK_SEPARATOR§§§
**Oluline:** Alati salvesta tühistamise funktsioon ja kutsu see `deactivate()`-s välja, et vältida mälu leket.

---

### `ctx.sftp` — Faili ülekandmine
...
Loetle failid kaugrepertuaaris.
§§§CHUNK_SEPARATOR§§§
#### `sftp.upload(sessionId, localPath, remotePath)`

Laadi fail kohalikult masinalt kaugreserverisse.
§§§CHUNK_SEPARATOR§§§
#### `sftp.download(sessionId, remotePath, localPath)`

Laadi fail kaugreserverist kohalikku masinasse.
§§§CHUNK_SEPARATOR§§§
...
§§§CHUNK_SEPARATOR§§§
**Ikooni viide:** Sirvi kõiki saadaolevaid ikoone aadressil [lucide.dev/icons](https://lucide.dev/icons)

> **Ühilduvuse märkus:** Mõned vanemad pluginad kasutavad positsioonilisi argumente nagu `addSidebarButton(id, icon, label, onClick)`. Ametlik API kasutab **valikute objekti**, nagu ülal dokumenteeritud. Uute pluginate puhul kasuta alati objekti stiili.
...
```
> Näe [Osa 3](#part-3-building-custom-ui-with-webviews) edistyneitä webview-malleja varten.
...
|-----------|--------|--------|
| `type` | `'success' \| 'error' \| 'info'` | Ilmoitustyyli |
...
Lisää pysyvä tekstikohde alalaidan tilariville.

| Parametri | Tyyppi | Kuvaus |
|-----------|--------|--------|
| `id` | `string` | Yksilöllinen ID tälle tilakohteelle |
...
```
Palauttaa `undefined`, jos avainta ei ole olemassa.
...
## Osa 3: Mukautetun UI:n rakentaminen Webviewilla

`openWebview()` API:n avulla voit rakentaa hallintapaneelin UI:ta HTML:n, CSS:n ja JavaScriptin avulla — kaikki popup-ikkunassa.

> **Tärkeä rajoitus:** Webviewt ovat **näyttö-vain**. Ne eivät voi kutsua takaisin lisäosan API:hin (`ctx.settings`, `ctx.terminal`, jne.). Käytä sivupainikkeita kaikissa käyttäjätoiminnoissa ja käytä `openWebview()` näyttämään nykyinen tila. Jos tarvitset interaktiivisia ominaisuuksia, aktivoi ne sivupainikkeista ja avaa webview uudelleen näyttöä päivittääksesi.

### Malli: Terminal Komento → Tuloksen Jäsentäminen → Näytä HTML:ssä

Tämä on yleisin lisäosamalli. Suoritat komennon, jäsennät tuloksen ja näytät sen visuaalisesti.
```javascript
...
> **Huom:** Webviewt ovat näyttö-vain — ne eivät voi kutsua takaisin lisäosan API:hin. Käytä `ctx.settings` sivupainikkeiden käsittelijöissä muuttaaksesi asetuksia ja käytä `openWebview()` näyttääksesi nykyisen tilan.
```javascript
...
3. Varmista, että se toimii: sivupainike ilmestyy, ominaisuudet toimivat oikein
4. Testaa äärimmäisiä tapauksia: mitä tapahtuu, jos terminaalia ei ole kytketty?
...
```
**Tarvittavat `package.json` kentät:**

| Kenttä | Kuvaus | Esimerkki |
|--------|--------|-----------|
| `name` | Yksilöllinen kebab-case ID | `"my-awesome-plugin"` |
| `version` | Semanttinen versio | `"1.0.0"` |
| `description` | Yhden lauseen kuvaus | `"Valvoo nginx pääsylokit reaaliajassa"` |
| `author` | Nimesi | `"John Doe"` |
| `main` | Alkuperäinen piste | `"index.js"` |

**Valinnaiset kentät:**

| Kenttä | Kuvaus |
|--------|--------|
| `license` | Lisenssityyppi (MIT suositeltava) |
| `keywords` | Hakutunnisteiden taulukko |
| `soom.minVersion` | Vähimmäis WIA SOOM versio vaaditaan |
...
Nämä säännöt ovat **pakollisia**. Pluginat, jotka rikkovat niitä, hylätään.
...
| **ÄLÄ KOSKAAN** käytä npm ulkoisia paketteja | Vain puhdasta JavaScriptiä — ei node_modules |
| **TÄYTYY** käyttää `ctx.terminal.send()` kaikille etäkomentoille | Tämä kulkee turvallisen SSH-kanavan kautta |
| **TÄYTYY** siivota `deactivate()` | Poista kuuntelijat, tyhjennä aikaväliä |
...
```
### Siivous deactivate() -toiminnossa
...
## Osa 6: Reaalimaailman esimerkit

### Esimerkki 1: Palvelimen levytilan tarkistaja
...
Plugin, joka hallitsee TODO-listaa käyttäen asetuksia pysyvään tallennukseen ja webview'ta näyttämiseen.

> **Suunnittelumalli:** Koska webview't eivät voi suoraan kutsua plugin API:ita, tämä plugin käyttää "snapshot" lähestymistapaa — se lukee TODO:t asetuksista, renderoi ne vain luku -HTML:ksi ja tarjoaa sivupalkkipohjaisia toimintoja kohteiden lisäämiseksi. Webview on **näyttö** kerros, ei interaktiivinen lomake.
```javascript
...
Valvoo terminaalin tulostetta ja lähettää ilmoituksen, kun tiettyjä kuvioita havaitaan.
```javascript
...
| `quantum` | Kvanttitietokonetyökalut |
...
- **Liitännäiset Ongelmat:** [GitHub Issues](https://github.com/WIA-Official/wiasoom.com/issues)
