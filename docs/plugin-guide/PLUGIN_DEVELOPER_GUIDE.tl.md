...
- [Bahagi 2: Sanggunian sa Plugin Context API](#bahagi-2-sanggunian-sa-plugin-context-api)
...
- [Bahagi 4: Pag-publish ng Iyong Plugin](#bahagi-4-pag-publish-ng-iyong-plugin)
...
- [Appendix: Mga Kategorya at Icon](#appendix-mga-kategorya-at-icon)
...
Isang "Hello World" na plugin na nagdadagdag ng isang button sa sidebar. Kapag na-click, ito ay nagpapakita ng isang notification.

### Hakbang 1: Gumawa ng folder para sa plugin
```bash
mkdir -p ~/.wia-soom/plugins/hello-world
cd ~/.wia-soom/plugins/hello-world
```
### Hakbang 2: Gumawa ng package.json
```json
{
  "name": "hello-world",
  "version": "1.0.0",
...
## Bahagi 2: Sanggunian sa Plugin Context API

Kapag ang iyong `activate(context)` function ay tinawag, ang `context` (o `ctx`) ay nagbibigay ng mga API na ito:
```typescript
...
Magpadala ng isang utos (o anumang data) sa isang aktibong terminal session.
...
| Parameter | Uri | Paglalarawan |
|-----------|------|-------------|
| `sessionId` | `string` | Ang terminal session na babantayan |
...
> **Katayuan: Darating na** — Ang SFTP API ay naitakda ngunit hindi pa nakakabit sa SFTP engine ng app. Ang `list()` ay kasalukuyang nagbabalik ng isang walang laman na array, at ang `upload()`/`download()` ay walang ginagawa. Ito ay ganap na ipapatupad sa isang hinaharap na release. Sa ngayon, gamitin ang `ctx.terminal.send()` kasama ang `scp` o `rsync` na mga utos bilang workaround.
...
Mag-upload ng isang file mula sa lokal na makina patungo sa remote server.
```javascript
...
Mag-download ng isang file mula sa remote server patungo sa lokal na makina.
```javascript
await context.sftp.download(sessionId, '/var/log/app.log', '/tmp/app.log');
```
**Workaround (hanggang ang SFTP API ay live):**
```javascript
...
Magdagdag ng isang button sa sidebar ng WIA SOOM.

| Opsyon | Uri | Kinakailangan | Paglalarawan |
|--------|------|----------|-------------|
| `id` | `string` | Hindi | Natatanging ID (default ay pangalan ng plugin) |
| `icon` | `string` | Oo | Pangalan ng icon ng Lucide (hal., `'server'`, `'shield'`, `'database'`) |
| `label` | `string` | Oo | Teksto ng button na ipinapakita sa sidebar |
| `onClick` | `() => void` | Oo | Function na tinatawag kapag ang button ay na-click |
```javascript
context.ui.addSidebarButton({
  id: 'my-dashboard',
...
```
**Sanggunian ng icon:** Tingnan ang lahat ng available na icons sa [lucide.dev/icons](https://lucide.dev/icons)

> **Tala ng pagkakatugma:** Ang ilang mga mas lumang plugin ay gumagamit ng positional arguments tulad ng `addSidebarButton(id, icon, label, onClick)`. Ang opisyal na API ay gumagamit ng isang **options object** tulad ng nakadokumento sa itaas. Palaging gamitin ang object style para sa mga bagong plugin.

#### `ui.openWebview(options)`

Buksan ang isang popup window na may custom na HTML content. Ganito mo binubuo ang mga mayamang UI.
...
Magdagdag ng isang persistent na item ng teksto sa ibabang status bar.

| Parameter | Uri | Paglalarawan |
|-----------|------|-------------|
| `id` | `string` | Natatanging ID para sa item na ito sa status |
...
> **Katayuan: Darating na** — Ang AI API ay tinukoy ngunit hindi pa nakakonekta sa Soomy. Sa kasalukuyan ay nagbabalik ng `{ response: 'AI not yet connected' }`. Ang buong integrasyon ng AI ay nakaplano para sa isang hinaharap na paglabas.
...
## Bahagi 3: Pagbuo ng Custom UI gamit ang Webviews
...
Ito ang pinaka-karaniwang pattern ng plugin. Nagpapatakbo ka ng isang utos, pinoproseso ang resulta, at ipinapakita ito nang biswal.
```javascript
exports.activate = function(context) {
  context.ui.addSidebarButton({
    icon: 'hard-drive',
...
> **Tandaan:** Ang mga webview ay display-only — hindi sila makatawag pabalik sa mga plugin API. Gumamit ng `ctx.settings` sa iyong mga sidebar button handler upang baguhin ang mga setting, at gamitin ang `openWebview()` upang ipakita ang kasalukuyang estado.
```javascript
...
## Bahagi 4: Pag-publish ng Iyong Plugin
...
### Hakbang 2: Ihanda para sa pagsusumite
...
```
**Kinakailangang `package.json` na mga patlang:**
...
Sinasala namin ang bawat plugin para sa:
...
3. Ang iyong plugin ay lumalabas sa **Plugin Store** para sa lahat ng mga gumagamit ng WIA SOOM!
...
| **HINDI KAILANMAN** i-access ang `process.env` | Ang mga environment variable ay maaaring maglaman ng mga lihim |
...
| **DAPAT** linisin sa `deactivate()` | Alisin ang mga listeners, linisin ang mga intervals |
...
Nagtatakbo ng `df -h` sa remote server at ipinapakita ang ginamit/available na espasyo sa status bar.
```javascript
'use strict';
...
Isang plugin na namamahala ng isang TODO list gamit ang mga setting para sa persistent storage at isang webview para sa display.

> **Disenyo ng pattern:** Dahil ang mga webview ay hindi maaaring direktang tumawag sa mga plugin APIs, ang plugin na ito ay gumagamit ng "snapshot" na diskarte — binabasa nito ang mga TODO mula sa mga setting, nire-render ang mga ito bilang read-only HTML, at nagbibigay ng mga sidebar-based na aksyon para sa pagdaragdag ng mga item. Ang webview ay isang **display** layer, hindi isang interactive na form.
```javascript
'use strict';
...
Nagmamasid sa output ng terminal at nagpapadala ng notification kapag ang mga tiyak na pattern ay natukoy.
```javascript
'use strict';
...
| `devtools` | Mga kasangkapan sa pag-unlad |
| `calculator` | Mga kalkulador at converter |
| `simulator` | Mga simulator |
| `game` | Mga laro sa terminal |
| `business` | Mga kasangkapan sa negosyo |
| `security` | Seguridad at pag-audit |
| `web` | Pamamahala ng web server |
| `education` | Mga kasangkapan sa edukasyon |
| `health` | Mga kasangkapan na may kaugnayan sa kalusugan |
| `islamic` | Mga kasangkapan ng Islam (mga oras ng dasal, atbp.) |
| `science` | Mga kasangkapan sa agham |
| `quantum` | Mga kasangkapan sa quantum computing |
| `ai` | Mga kasangkapan na pinapagana ng AI |
| `biotech` | Mga kasangkapan sa biotechnology |
| `space` | Mga kasangkapan sa espasyo at astronomiya |
| `network` | Mga kasangkapan sa network |
| `database` | Pamamahala ng database |
| `monitoring` | Pagsubaybay sa server |
| `devops` | DevOps at CI/CD |
| `utility` | Pangkalahatang mga utility |
| `design` | Mga kasangkapan sa disenyo |
| `ecommerce` | Mga kasangkapan sa e-commerce |
| `automation` | Mga kasangkapan sa automation |
| `kpop` | Mga kasangkapan na may kaugnayan sa K-pop |
| `accessibility` | Mga kasangkapan sa accessibility |
| `analytics` | Analytics at pag-uulat |
| `wia` | Mga kasangkapan ng WIA ecosystem |
| `all` | Lumalabas sa lahat ng kategorya |

### Inirerekomendang Icons (Lucide)

| Pangalan ng Icon | Gamitin para sa |
|------------------|------------------|
| `server` | Pamamahala ng server |
| `shield` | Seguridad |
| `database` | Database |
| `activity` | Pagsubaybay |
| `terminal` | Mga kasangkapan sa terminal |
| `code` | Pag-unlad |
| `hard-drive` | Disk/storage |
| `network` | Networking |
| `lock` | Auth/encryption |
| `eye` | Pagsubaybay/pagmamasid |
...
Tingnan ang lahat ng 1,500+ icons: [lucide.dev/icons](https://lucide.dev/icons)
