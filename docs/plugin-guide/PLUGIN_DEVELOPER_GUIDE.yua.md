...
<p align="center">Crea herramientas poderosas para servidores, tableros y automatizaciones — directamente dentro de WIA SOOM.</p>
...
| **Devuelve** | `() => void` | Llama esto para dejar de escuchar |
```javascript
...
> **Estado: Próximamente** — La API de SFTP está definida pero aún no conectada al motor SFTP de la aplicación. `list()` actualmente devuelve un arreglo vacío, y `upload()`/`download()` no hacen nada. Esto se implementará completamente en una futura versión. Por ahora, utiliza `ctx.terminal.send()` con comandos `scp` o `rsync` como solución alternativa.
...
```
> K'a'abéet [Part 3](#part-3-building-custom-ui-with-webviews) jump'éel patrones webview más avanzados.

#### `ui.showNotification(type, message)`

Muestra jump'éel toast notification.

| Parameter | Type | Description |
|-----------|------|-------------|
| `type` | `'success' \| 'error' \| 'info'` | Estilo de la notificación |
| `message` | `string` | Texto para mostrar |
```javascript
...
Añade jump'éel ítem de texto persistente a la barra de estado inferior.

| Parameter | Type | Description |
|-----------|------|-------------|
| `id` | `string` | ID único para este ítem de estado |
...
Los ajustes del plugin se almacenan permanentemente en `~/.wia-soom/plugins/{your-plugin}/.plugin-settings.json`.
...
> **Estado: Pr��ximamente** — La API de IA está definida pero aún no está conectada a Soomy. Actualmente devuelve `{ response: 'AI not yet connected' }`. Se planea una integración completa de IA para una futura versión.
...
## Part 3: Construyendo UI Personalizada con Webviews
...
> **Nota:** Los webviews son solo de visualización — no pueden llamar a las APIs del plugin. Usa `ctx.settings` en tus manejadores de botones de la barra lateral para modificar ajustes, y usa `openWebview()` para mostrar el estado actual.
```javascript
...
4. Prueba casos límite: ¿qué sucede si no hay terminal conectada?
...
```
**K'a'ab' `package.json` k'i'ik' fields:**

| K'i'ik' | Descripción | Ejemplo |
|-------|-------------|---------|
...
| **NUNCA** obtengas URLs externas | Excepción: puntos finales de la API de `wiasoom.com` |
...
| **DEBES** limpiar en `deactivate()` | Elimina oyentes, limpia intervalos |
...
## Appendix: Kategoorías & Ikonos

### Kategoorías de Plugin (29)

Usa estas en tu `package.json` `keywords` o al enviar a la registraduría:

| Kategooría | Descripción |
|------------|-------------|
| `server` | Manejo general del servidor |
...
| `database` | Manejo de bases de datos |
...
### Ikonos Recomendados (Lucide)

| Nombre del Ícono | Usar para |
|------------------|-----------|
| `server` | Manejo del servidor |
| `shield` | Seguridad |
...
- **Problemas de Plugin:** [GitHub Issues](https://github.com/WIA-Official/wiasoom.com/issues)
