...
<p align="center"><strong>5分钟内构建您自己的插件。</strong></p>
...
- [第1部分：快速入门 — 您的第一个插件在5分钟内](#part-1-quick-start--your-first-plugin-in-5-minutes)
- [第2部分：插件上下文 API 参考](#part-2-plugin-context-api-reference)
  - [ctx.terminal](#ctxterminal--run-commands-on-remote-servers)
  - [ctx.sftp](#ctxsftp--file-transfer)
  - [ctx.ui](#ctxui--user-interface)
  - [ctx.settings](#ctxsettings--persistent-storage)
  - [ctx.ai](#ctxai--ai-integration)
- [第3部分：使用 Webviews 构建自定义 UI](#part-3-building-custom-ui-with-webviews)
- [第4部分：发布您的插件](#part-4-publishing-your-plugin)
- [第5部分：最佳实践](#part-5-best-practices)
- [第6部分：真实世界示例](#part-6-real-world-examples)
- [附录：类别与图标](#appendix-categories--icons)

---

## 第1部分：快速入门 — 您的第一个插件在5分钟内

### 您将构建的内容

一个“Hello World”插件，它在侧边栏添加一个按钮。点击时，它会显示一个通知。
...
重启应用程序（或在设置 → 插件中切换插件的开/关）。

您应该在侧边栏看到一个 **"Hello World"** 按钮。点击它 — 您会看到一个成功的通知！
...
当您的 `activate(context)` 函数被调用时，`context`（或 `ctx`）提供这些 API：
§§§CHUNK_SEPARATOR§§§
---
...
> **状态：即将推出** — SFTP API 已定义，但尚未连接到应用程序的 SFTP 引擎。`list()` 当前返回一个空数组，`upload()`/`download()` 是无操作。这将在未来的版本中完全实现。现在，使用 `ctx.terminal.send()` 结合 `scp` 或 `rsync` 命令作为解决方法。
...
| `onClick` | `() => void` | 是 | 点击按钮时调用的函数 |
§§§CHUNK_SEPARATOR§§§
**图标参考：** 在 [lucide.dev/icons](https://lucide.dev/icons) 浏览所有可用图标

> **兼容性说明：** 一些旧插件使用位置参数，如 `addSidebarButton(id, icon, label, onClick)`。官方 API 使用如上所述的 **选项对象**。新插件始终使用对象样式。
...
```
**Yauz `package.json` fwenj:**

| Fwenj | Gwn | Yauz |
|-------|-------------|---------|
| `name` | Yenz goengq kebab-case ID | `"my-awesome-plugin"` |
| `version` | Semantc fwenj | `"1.0.0"` |
| `description` | Yienh-lai gwn | `"Monitors nginx access logs in real-time"` |
| `author` | Ndeic | `"John Doe"` |
| `main` | Yenz goengq | `"index.js"` |

**Bwnj fwenj:**

| Fwenj | Gwn |
|-------|-------------|
| `license` | License type (MIT gwnz) |
| `keywords` | Gvicanj goengq gwnz | 
| `soom.minVersion` | WIA SOOM fwenj gwnz goengq |

### Step 3: Gvicanj goengq Plugin Registry

1. ****Package** your plugin as a ZIP file
2. **Add** neix plugin go `plugins/{your-plugin-name}/`
3. **Submit** a Pull Request

### Step 4: Gvicanj raemx goengq

Raemx goengq gwnz neix plugin:

- **Security** — mbouj goengq API gwnz (daengz [Security Rules](#security-rules))
- **Quality** — neix gwn? Yienh goengq gwnz?
- **Usefulness** — neix gwnz goengq raemx gwnz?

Raemx goengq:
1. Neix plugin goengq `registry.json`
2. A ZIP goengq go `dist/`
3. Neix plugin goengq raemx go **Plugin Store** go daengz WIA SOOM yienh!
...
Neix gwnz **daengz**. Plugins gwnz mbouj gwnz goengq raemx gwnz.

| Gwn | Yienh |
|------|-----|
| **NEVER** goengq `eval()` raemx `new Function()` | Gwnz goengq gwnz |
| **NEVER** goengq `child_process`, `exec()`, `spawn()` | Yienh goengq `ctx.terminal.send()` go gwnz |
| **NEVER** goengq external URLs | Gwnz: `wiasoom.com` API endpoints |
| **NEVER** goengq `process.env` | Gwnz gwnz goengq gwnz gwnz |
| **NEVER** goengq `require('fs')` goengq | Yienh goengq `ctx.settings` go gwnz, `ctx.sftp` go gwnz |
| **NEVER** goengq npm external packages | Pure JavaScript goengq — mbouj node_modules |
| **MUST** goengq `ctx.terminal.send()` go gwnz remote gwnz | Neix goengq raemx goengq SSH gwnz |
| **MUST** gwnz goengq `deactivate()` | Gwnz listeners, gwnz intervals |

### Error Handling

Yienh gwnz gwnz gwnz raemx try/catch:
```javascript
context.ui.addSidebarButton({
  id: 'my-dashboard',
...
```
### Gvicanj goengq go deactivate()

Raeuj neix plugin gwnz gwnz intervals, listeners, raemx subscriptions — gvicanj goengq:
```javascript
context.ui.openWebview({
  title: 'Server Status',
  html: `
...
WIA SOOM gvicanj 254 gwnz. Yienh gwnz plugin label gwnz goengq, yienh gwnz gwnz:
```javascript
...
## Part 6: Raemx Gwnz Gvicanj

### Raemx 1: Server Disk Checker

Raemx `df -h` go remote server raemx gwnz used/available space go status bar.
```javascript
...
### Raemx 2: TODO Manager

Neix plugin gwnz gvicanj a TODO list yienh gwnz settings go persistent storage raemx a webview go display.

> **Design pattern:** Yienh webviews mbouj goengq goengq plugin APIs, neix plugin gwnz yienh "snapshot" gwnz — neix raemx TODOs go settings, yienh goengq raemx read-only HTML, raemx gwnz sidebar-based actions go gwnz items. Neix webview goengq raemx go **display** layer, mbouj goengq interactive form.
```javascript
...
### Raemx 3: Error Watcher

Monitors terminal output raemx sends a notification raemx specific patterns goengq.
```javascript
...
在你的 `package.json` `keywords` 中使用这些类别，或在提交到注册表时使用：
