...
<p align="center"><strong>Baue dein eigenes Plugin in 5 Minuten.</strong></p>
...
- [Teil 2: Plugin Kontext API Referenz](#teil-2-plugin-kontext-api-referenz)
  - [ctx.terminal](#ctxterminal--befehle-auf-remoteservern-ausführen)
...
- [Teil 4: Veröffentlichen deines Plugins](#teil-4-veröffentlichen-deines-plugins)
...
### `ctx.terminal` — Befehle auf Remoteservern ausführen
...
| Parameter | Typ | Beschreibung |
|-----------|------|-------------|
| `sessionId` | `string` | Die Terminal-Sitzung, die beobachtet werden soll |
...
> **Status: Bald verfügbar** — Die SFTP API ist definiert, aber noch nicht an die SFTP-Engine der App angeschlossen. `list()` gibt derzeit ein leeres Array zurück, und `upload()`/`download()` sind No-ops. Dies wird in einer zukünftigen Version vollständig implementiert. Für den Moment verwende `ctx.terminal.send()` mit `scp` oder `rsync` Befehlen als Workaround.
...
Lade eine Datei vom lokalen Rechner auf den Remote-Server hoch.
```javascript
...
Lade eine Datei vom Remote-Server auf den lokalen Rechner herunter.
```javascript
...
Die API `openWebview()` ermöglicht es Ihnen, Dashboard-UIs mit HTML, CSS und JavaScript zu erstellen — alles in einem Popup-Fenster.
...
2. Starten Sie WIA SOOM neu
3. Überprüfen Sie, ob es funktioniert: Sidebar-Button erscheint, Funktionen arbeiten korrekt
4. Testen Sie Grenzfälle: Was passiert, wenn kein Terminal verbunden ist?
...
### Schritt 3: Einreichung im Plugin-Registry

1. ****Package** your plugin as a ZIP file
2. **Füge** dein Plugin zu `plugins/{dein-plugin-name}/` hinzu
...
## Teil 5: Beste Praktiken

### Sicherheitsregeln

Diese Regeln sind **verpflichtend**. Plugins, die gegen sie verstoßen, werden abgelehnt.

| Regel | Warum |
|------|-----|
| **NIE** `eval()` oder `new Function()` verwenden | Risiko von Code-Injektionen |
...
| **NIE** `require('fs')` direkt verwenden | Verwende `ctx.settings` für Speicherung, `ctx.sftp` für Dateiübertragungen |
...
Wickle risikobehaftete Operationen immer in try/catch:
```javascript
exports.activate = function(context) {
  try {
    // Your plugin logic
...
## Appendix: Kategorien & Icons

### Plugin-Kategorien (29)

Verwenden Sie diese in Ihrer `package.json` `keywords` oder beim Einreichen im Register:
...
| `space` | Raumfahrt- und Astronomietools |
| `network` | Netzwerktools |
