...
<h1 align="center">Guide di Développeu di Plugin WIA SOOM</h1>
...
- [Partie 2 : Référence de l'API de Contexte de Plugin](#partie-2-référence-de-lapi-de-contexte-de-plugin)
...
Redémarrez l'application (ou basculez le plugin hors ligne/en ligne dans Paramètres → Plugins).
...
## Partie 2 : Référence de l'API de Contexte de Plugin
...
S'abonner à toute sortie d'une session de terminal. Renvoie une **fonction de désinscription**.

| Paramètre | Type | Description |
|-----------|------|-------------|
| `sessionId` | `string` | La session de terminal à surveiller |
| `callback` | `(data: string) => void` | Appelé avec chaque morceau de sortie |
| **Renvoie** | `() => void` | Appelez ceci pour arrêter d'écouter |
§§§CHUNK_SEPARATOR§§§
**Important :** Toujours sauvegarder la fonction de désinscription et l'appeler dans `deactivate()` pour éviter les fuites de mémoire.

---

### `ctx.sftp` — Transfert de fichiers

> **Statut : À Venir** — L'API SFTP est définie mais pas encore connectée au moteur SFTP de l'application. `list()` renvoie actuellement un tableau vide, et `upload()`/`download()` ne font rien. Cela sera entièrement implémenté dans une future version. Pour l'instant, utilisez `ctx.terminal.send()` avec des commandes `scp` ou `rsync` comme solution de contournement.
...
Ouvrir une fenêtre contextuelle avec du contenu HTML personnalisé. C'est ainsi que vous construisez des UIs riches.
...
```
> Veyez [Part 3](#part-3-building-custom-ui-with-webviews) po des patterns avansés di webview.

#### `ui.showNotification(type, message)`

Fait on toast notification.

| Parameter | Type | Description |
|-----------|------|-------------|
| `type` | `'success' \| 'error' \| 'info'` | Style di notification |
| `message` | `string` | Tchôx a montrer |
```json
{
  "name": "hello-world",
  "version": "1.0.0",
...
Ajoute on item di texte persistint al ba di la barre di statut.

| Parameter | Type | Description |
|-----------|------|-------------|
| `id` | `string` | ID unique po c' item di statut |
| `text` | `string` | Tchôx a afficher |
```javascript
'use strict';
...
### `ctx.settings` — Stockage persistint

Les settings di plugin sont stockés permanint di `~/.wia-soom/plugins/{your-plugin}/.plugin-settings.json`.

#### `settings.get(key)`

Li on valêye sauvgardé.
```
...
```
Rèturne `undefined` si l' clé n' existe pas.

#### `settings.set(key, value)`

Sauvgarde on valêye. Supporte les strings, numbres, booleans, arrays, et objects.
```typescript
...
```
**Exemple: Rémindr les préférences di l' utilisateur**
```javascript
...
> **Statut: Vînt tot** — L' API AI est définie mais n' est nén co connectée a Soomy. Rèturne actuellement `{ response: 'AI not yet connected' }`. L' intégration complète di l' AI est planifiée po on future release.

#### `ai.chat(messages, options?)`

Envoie des messages a l' assistant AI (Soomy).
```javascript
...
## Part 3: Bâtir on UI Personnalisé avc des Webviews

L' API `openWebview()` te permet di bâtir des UIs di dashboard avc HTML, CSS, et JavaScript — tot dins on popup.

> **Limitation importante:** Les webviews sont **uniquement diisplay**. I n' peuvent nén apeler les APIs di plugin (`ctx.settings`, `ctx.terminal`, etc.). Utilise des boutons di sidebar po tot les actions di l' utilisateur, et utilise `openWebview()` po montrer l' état courant. Si t' as besoin di fonctionnalités interactives, déclenche-les di boutons di sidebar et rouvres l' webview po rafraîchir l' affichage.

### Pattern: Commande di Terminal → Parser l' Sortie → Montrer en HTML

C' est l' pattern di plugin le plus commun. T' exécutes on commande, parses l' résultat, et l' affiches visuellement.
```javascript
var files = await context.sftp.list(sessionId, '/var/log/');
// files = [{ name: 'syslog', size: 1024, ... }, ...]
```
### Pattern: Dashboard Interactif avc Auto-Rafraîchissement
```javascript
await context.sftp.upload(sessionId, '/tmp/config.json', '/etc/myapp/config.json');
context.ui.showNotification('success', 'Config uploaded!');
```
### Pattern: Montrer les Settings dins on Webview

> **Nôte:** Les webviews sont uniquement diisplay — i n' peuvent nén apeler les APIs di plugin. Utilise `ctx.settings` dins tes handlers di boutons di sidebar po modifier les settings, et utilise `openWebview()` po montrer l' état courant.
```javascript
await context.sftp.download(sessionId, '/var/log/app.log', '/tmp/app.log');
```
---

## Part 4: Publier T' Plugin

### Étape 1: Tester localement

1. Copie t' plugin a `~/.wia-soom/plugins/{your-plugin}/`
2. Redémarre WIA SOOM
3. Vérifie si ça marche: le bouton di sidebar apparaît, les fonctionnalités marchent correctement
4. Teste les cas limites: qu' est-ce qui arrive si nén on terminal est connecté?

### Étape 2: Préparer po la soumission

T' dossier di plugin doit contenir:
```javascript
// Use terminal commands instead
context.terminal.send(sessionId, 'scp user@host:/var/log/app.log /tmp/\n');
```
**Champs `package.json` nécéssaires :**
...
| `keywords` | Tableau de mots-clés de recherche |
...
| **DOIT** utiliser `ctx.terminal.send()` pour toutes les commandes à distance | Cela passe par le canal SSH sécurisé |
...
Utilisez ceux-ci dans vos `package.json` `keywords` ou lors de la soumission au registre :
...
| `space` | Outils pour l'espace et l'astronomie |
