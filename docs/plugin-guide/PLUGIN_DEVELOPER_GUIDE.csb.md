...
<p align="center"><strong>Zbuduj swoją własną wtyczkę w 5 minut.</strong></p>
...
- [Część 2: Referencja API kontekstu wtyczki](#część-2-referencja-api-kontekstu-wtyczki)
  - [ctx.terminal](#ctxterminal--uruchamiaj-komendy-na-zdalnych-serwerach)
...
- [Część 4: Publikowanie swojej wtyczki](#część-4-publikowanie-swojej-wtyczki)
...
Uruchom ponownie aplikację (lub przełącz wtyczkę w ustawieniach → Wtyczki).
...
Gdy wywołana zostanie twoja funkcja `activate(context)`, `context` (lub `ctx`) udostępnia te API:
```typescript
...
### `ctx.terminal` — Uruchamiaj komendy na zdalnych serwerach

#### `terminal.send(sessionId, data)`

Wyślij komend�� (lub jakiekolwiek dane) do aktywnej sesji terminala.
...
Subskrybuj wszystkie wyjścia z sesji terminala. Zwraca **funkcję wypisania**.

| Parametr | Typ | Opis |
|----------|-----|------|
| `sessionId` | `string` | Sesja terminala, którą obserwujesz |
...
unsubscribe();
```
**Ważne:** Zawsze zapisz funkcję wypisania i wywołaj ją w `deactivate()`, aby zapobiec wyciekom pamięci.

---

### `ctx.sftp` — Transfer plików

> **Status: Wkrótce** — API SFTP jest zdefiniowane, ale jeszcze nie podłączone do silnika SFTP aplikacji. `list()` obecnie zwraca pustą tablicę, a `upload()`/`download()` nie robią nic. To będzie w pełni zaimplementowane w przyszłej wersji. Na razie użyj `ctx.terminal.send()` z komendami `scp` lub `rsync` jako obejście.
...
Prześlij plik z lokalnej maszyny na zdalny serwer.
```javascript
...
Pobierz plik z zdalnego serwera na lokalną maszynę.
```javascript
...
> **Uwaga dotycząca kompatybilności:** Niektóre starsze wtyczki używają argumentów pozycyjnych, takich jak `addSidebarButton(id, icon, label, onClick)`. Oficjalne API używa **obiektu opcji**, jak opisano powyżej. Zawsze używaj stylu obiektowego dla nowych wtyczek.
...
> **Status: Wkrótce** — API AI jest zdefiniowane, ale jeszcze nie połączone z Soomy. Obecnie zwraca `{ response: 'AI not yet connected' }`. Pełna integracja AI jest planowana na przyszłe wydanie.
...
> **Ważne ograniczenie:** Webviews są **tylko do wyświetlania**. Nie mogą wywoływać API wtyczek (`ctx.settings`, `ctx.terminal`, itp.). Użyj przycisków bocznych do wszystkich działań użytkownika i użyj `openWebview()`, aby wyświetlić bieżący stan. Jeśli potrzebujesz interaktywnych funkcji, wyzwól je z przycisków bocznych i ponownie otwórz webview, aby odświeżyć wyświetlanie.

### Wzorzec: Komenda terminala → Parsowanie wyjścia → Wyświetlanie w HTML

To najczęstszy wzorzec wtyczki. Uruchamiasz komendę, parsujesz wynik i wyświetlasz go wizualnie.
```javascript
exports.activate = function(context) {
  context.ui.addSidebarButton({
    icon: 'hard-drive',
...
> **Uwaga:** Webviews są tylko do wyświetlania — nie mogą wywoływać API wtyczek. Użyj `ctx.settings` w swoich handlerach przycisków bocznych, aby modyfikować ustawienia, i użyj `openWebview()`, aby pokazać bieżący stan.
```javascript
...
3. Sprawdź, czy działa: przycisk boczny się pojawia, funkcje działają poprawnie
4. Testuj przypadki brzegowe: co się stanie, jeśli żaden terminal nie jest podłączony?

### Krok 2: Przygotuj do złożenia
...
| `description` | Opis w jednej linii | `"Monitors nginx access logs in real-time"` |
...
### Krok 3: Zgłoś do Rejestru Wtyczek
...
3. Twoja wtyczka pojawia się w **Plugin Store** dla wszystkich użytkowników WIA SOOM!
...
Te zasady są **obowiązkowe**. Wtyczki, które je łamią, będą odrzucane.

| Zasada | Dlaczego |
|--------|----------|
| **NIGDY** nie używaj `eval()` lub `new Function()` | Ryzyko wstrzykiwania kodu |
...
| **MUSISZ** używać `ctx.terminal.send()` do wszystkich poleceń zdalnych | To przechodzi przez bezpieczny kanał SSH |
...
WIA SOOM wspiera 254 języki. Aby uczynić etykietę twojej wtyczki tłumaczalną, użyj prostego podejścia:
§§§CHUNK_SEPARATOR§§§
---

## Część 6: Przykłady z Życia Wzięte

### Przykład 1: Sprawdzacz Dysku Serwera

Uruchamia `df -h` na zdalnym serwerze i pokazuje użyte/dostępne miejsce na pasku stanu.
§§§CHUNK_SEPARATOR§§§
---
...
## Dodatek: Kategórje & Ikony

### Kategórje Pluginów (29)

Używaj ich w swoim `package.json` `keywords` lub przy wysyłaniu do rejestru:
...
- **Problemy GitHub:** [wia-soom/issues](https://github.com/WIA-Official/wiasoom.com/issues)
- **Problemy Pluginów:** [GitHub Issues](https://github.com/WIA-Official/wiasoom.com/issues)
- **Przykładowe Pluginy:** [Website](https://wiasoom.com)
- **Strona Internetowa:** [wiasoom.com](https://wiasoom.com)

---

<p align="center"><em>Zbuduj coś niesamowitego. Podziel się tym ze światem.</em></p>
