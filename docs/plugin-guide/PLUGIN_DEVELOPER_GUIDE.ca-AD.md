...
<p align="center">Crea poderoses eines de servidor, panells de control i automatitzacions — directament dins de WIA SOOM.</p>
...
- [Part 2: Referència de l'API del Context del Plugin](#part-2-referència-de-lapi-del-context-del-plugin)
  ...
  - [ctx.ai](#ctxai--integració-dai)
- [Part 3: Construcció d'UI Personalitzada amb Webviews](#part-3-construcció-dui-personalitzada-amb-webviews)
...
Un plugin de "Hola món" que afegeix un botó a la barra lateral. Quan es fa clic, mostra una notificació.
...
Hauries de veure un botó **"Hola món"** a la barra lateral. Fes-hi clic — veuràs una notificació d'èxit!

### Com funciona
§§§CHUNK_SEPARATOR§§§
---

## Part 2: Referència de l'API del Context del Plugin
...
| **Retorna** | `() => void` | Crida això per deixar de escoltar |
§§§CHUNK_SEPARATOR§§§
**Important:** Sempre desa la funció de cancel·lar subscripció i crida-la a `deactivate()` per prevenir fuites de memòria.

---

### `ctx.sftp` — Transferència de fitxers

> **Estat: Properament** — L'API SFTP està definida però encara no està connectada al motor SFTP de l'aplicació. `list()` actualment retorna un array buit, i `upload()`/`download()` no fan res. Això s'implementarà completament en una futura versió. Per ara, utilitza `ctx.terminal.send()` amb comandes `scp` o `rsync` com a solució alternativa.
...
```
> Veure [Part 3](#part-3-building-custom-ui-with-webviews) per patrons avançats de webview.
...
Les configuracions del plugin es guarden de manera permanent a `~/.wia-soom/plugins/{your-plugin}/.plugin-settings.json`.
...
Guarda un valor. Admet cadenes, nombres, booleans, arrays i objectes.
```typescript
...
```
**Exemple: Recordar les preferències de l'usuari**
```javascript
...
> **Estat: Properament** — L'API d'IA està definida però encara no està connectada a Soomy. Actualment retorna `{ response: 'AI not yet connected' }`. La integració completa d'IA està prevista per a una futura versió.
...
## Part 3: Construcció d'UI Personalitzada amb Webviews
...
> **Limitació important:** Els webviews són **només de visualització**. No poden fer crides a les APIs del plugin (`ctx.settings`, `ctx.terminal`, etc.). Utilitza botons de barra lateral per a totes les accions de l'usuari, i utilitza `openWebview()` per mostrar l'estat actual. Si necessites funcions interactives, activa-les des dels botons de la barra lateral i torna a obrir el webview per actualitzar la visualització.

### Patró: Comandament del Terminal → Analitzar Sortida → Mostrar en HTML
...
```
### Patró: Tauler de Control Interactiu amb Actualització Automàtica
```javascript
await context.sftp.upload(sessionId, '/tmp/config.json', '/etc/myapp/config.json');
context.ui.showNotification('success', 'Config uploaded!');
```
### Patró: Mostrant Configuracions en un Webview

> **Nota:** Els webviews són només de visualització — no poden fer crides a les APIs del plugin. Utilitza `ctx.settings` en els teus controladors de botons de barra lateral per modificar configuracions, i utilitza `openWebview()` per mostrar l'estat actual.
```javascript
...
### Pas 2: Preparar per a la presentació
...
### Pas 3: Envia al registre de plugins
...
| Regla | Per què |
|-------|---------|
| **MAI** utilitzar `eval()` o `new Function()` | Risc d'injecció de codi |
| **MAI** utilitzar `child_process`, `exec()`, `spawn()` | Només utilitzar `ctx.terminal.send()` per a comandes |
| **MAI** obtenir URLs externes | Excepció: punts d'API de `wiasoom.com` |
| **MAI** accedir a `process.env` | Les variables d'entorn poden contenir secrets |
| **MAI** utilitzar `require('fs')` directament | Utilitzar `ctx.settings` per emmagatzematge, `ctx.sftp` per transferència de fitxers |
| **MAI** utilitzar paquets externs de npm | Només JavaScript pur — sense node_modules |
| **HA DE** utilitzar `ctx.terminal.send()` per a totes les comandes remotes | Això passa pel canal SSH segur |
| **HA DE** netejar a `deactivate()` | Eliminar listeners, netejar intervals |

### Gestió d'Errors

Sempre embolica operacions arriscades en try/catch:
```javascript
context.ui.addSidebarButton({
  id: 'my-dashboard',
...
### Exemple 1: Comprovador de Disc del Servidor
...
> **Patró de disseny:** Ja que els webviews no poden cridar directament les APIs dels plugins, aquest plugin utilitza un enfocament de "snapshot" — llegeix els TODOs de les configuracions, els renderitza com a HTML només de lectura, i proporciona accions basades en la barra lateral per afegir elements. El webview és una capa de **visualització**, no un formulari interactiu.
```javascript
...
Utilitzeu aquestes en el vostre `package.json` `keywords` o quan presenteu a la registre:
...
| `ai` | Eines amb intel·ligència artificial |
...
| `ecommerce` | Eines d'e-commerce |
| `automation` | Eines d'automatització |
| `kpop` | Eines relacionades amb el K-pop |
| `accessibility` | Eines d'accessibilitat |
| `analytics` | Analítica i informes |
...
Explora tots els 1,500+ icons: [lucide.dev/icons](https://lucide.dev/icons)
