...
<p align="center"><strong>Maak je eigen plugin in 5 minuten.</strong></p>
<p align="center">Creëer krachtige servertools, dashboards en automatiseringen — direct binnen WIA SOOM.</p>
...
- [Deel 2: Plugin Context API Referentie](#deel-2-plugin-context-api-referentie)
  - [ctx.terminal](#ctxterminal--voer-opdrachten-uit-op-afstandsservers)
...
### `ctx.terminal` — Voer opdrachten uit op afstandservers

#### `terminal.send(sessionId, data)`

Stuur een opdracht (of gegevens) naar een actieve terminalsessie.

| Parameter | Type | Beschrijving |
|-----------|------|--------------|
| `sessionId` | `string` | De terminalsessie om naar te verzenden |
| `data` | `string` | De opdracht of gegevens om te verzenden |
```javascript
...
Abonneer je op alle uitvoer van een terminalsessie. Retourneert een **afmeldfunctie**.

| Parameter | Type | Beschrijving |
|-----------|------|--------------|
| `sessionId` | `string` | De terminalsessie om te volgen |
| `callback` | `(data: string) => void` | Wordt aangeroepen met elke chunk uitvoer |
...
> **Status: Binnenkort Beschikbaar** — De SFTP API is gedefinieerd maar nog niet verbonden met de SFTP-engine van de app. `list()` retourneert momenteel een lege array, en `upload()`/`download()` zijn no-ops. Dit zal volledig worden geïmplementeerd in een toekomstige release. Gebruik voorlopig `ctx.terminal.send()` met `scp` of `rsync` opdrachten als een workaround.
...
|-------|------|---------|--------------|
| `id` | `string` | Nee | Unieke ID (standaard is de pluginnaam) |
| `icon` | `string` | Ja | Lucide icoonnaam (bijv. `'server'`, `'shield'`, `'database'`) |
...
```
**Icoonreferentie:** Blader door alle beschikbare iconen op [lucide.dev/icons](https://lucide.dev/icons)
...
Open een popupvenster met aangepaste HTML-inhoud. Dit is hoe je rijke UIs bouwt.
...
Toon een toastmelding.

| Parameter | Type | Beschrijving |
|-----------|------|--------------|
| `type` | `'success' \| 'error' \| 'info'` | Stijl van de melding |
...
De `openWebview()` API stelt je in staat om dashboard-UIs te bouwen met HTML, CSS en JavaScript — allemaal in een pop-upvenster.

> **Belangrijke beperking:** Webviews zijn **alleen ter weergave**. Ze kunnen geen terugroepacties naar plugin-API's aanroepen (`ctx.settings`, `ctx.terminal`, enz.). Gebruik zijbalkknoppen voor alle gebruikersacties en gebruik `openWebview()` om de huidige status weer te geven. Als je interactieve functies nodig hebt, activeer ze dan vanuit zijbalkknoppen en heropen de webview om de weergave te vernieuwen.
...
> **Opmerking:** Webviews zijn alleen ter weergave — ze kunnen geen terugroepacties naar plugin-API's aanroepen. Gebruik `ctx.settings` in je zijbalkknophandlers om instellingen te wijzigen, en gebruik `openWebview()` om de huidige status weer te geven.
```javascript
...
### Stap 3: Indienen bij het Plugin Register

1. ****Package** your plugin as a ZIP file
2. **Voeg** je plugin toe aan `plugins/{jouw-plugin-naam}/`
...
Na goedkeuring:
1. Je plugin wordt toegevoegd aan `registry.json`
2. Een ZIP-bundel wordt aangemaakt in `dist/`
3. Je plugin verschijnt in de **Plugin Store** voor alle WIA SOOM gebruikers!
...
Deze regels zijn **verplicht**. Plugins die deze schenden worden afgewezen.
...
| **NOOIT** npm externe pakketten gebruiken | Alleen pure JavaScript — geen node_modules |
...
Als je plugin intervallen, listeners of abonnementen aanmaakt — ruim ze op:
```javascript
var intervals = [];
var unsubscribers = [];
...
WIA SOOM ondersteunt 254 talen. Om je pluginlabel vertaalbaar te maken, gebruik je een eenvoudige aanpak:
```javascript
...
Bewaart de terminaluitvoer en stuurt een melding wanneer specifieke patronen worden gedetecteerd.
```javascript
'use strict';
...
Gebruik deze in je `package.json` `keywords` of bij het indienen bij het register:

| Categorie | Beschrijving |
|-----------|--------------|
| `server` | Algemeen serverbeheer |
...
| `web` | Webserverbeheer |
| `education` | Onderwijsgerelateerde tools |
