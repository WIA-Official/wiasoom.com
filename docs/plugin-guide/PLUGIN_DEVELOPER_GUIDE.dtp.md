...
- [Bahagian 2: Rujukan API Konteks Plugin](#bahagian-2-rujukan-api-konteks-plugin)
  - [ctx.terminal](#ctxterminal--jalankan-perintah-di-pelayan-jarak-jauh)
...
- [Bahagian 3: Membangun UI Khusus dengan Webviews](#bahagian-3-membangun-ui-khusus-dengan-webviews)
- [Bahagian 4: Menerbitkan Plugin Anda](#bahagian-4-menerbitkan-plugin-anda)
- [Bahagian 5: Amalan Terbaik](#bahagian-5-amalan-terbaik)
- [Bahagian 6: Contoh Dunia Nyata](#bahagian-6-contoh-dunia-nyata)
...
Plugin "Hello World" yang menambah butang ke bar sisi. Apabila diklik, ia menunjukkan notifikasi.
...
Mulakan semula aplikasi (atau togol plugin di luar/ke dalam di Tetapan → Plugin).

Anda seharusnya melihat butang **"Hello World"** di bar sisi. Klik ia — anda akan melihat notifikasi kejayaan!
...
### `ctx.terminal` — Jalankan perintah di pelayan jarak jauh
...
| `callback` | `(data: string) => void` | Dipanggil dengan setiap kepingan output |
...
unsubscribe();
```
**Penting:** Sentiasa simpan fungsi berhenti melanggan dan panggilnya dalam `deactivate()` untuk mengelakkan kebocoran memori.

---

### `ctx.sftp` — Pemindahan fail

> **Status: Akan Datang** — API SFTP telah ditakrifkan tetapi belum disambungkan ke enjin SFTP aplikasi. `list()` pada masa ini mengembalikan array kosong, dan `upload()`/`download()` tidak berfungsi. Ini akan dilaksanakan sepenuhnya dalam keluaran akan datang. Untuk sekarang, gunakan `ctx.terminal.send()` dengan perintah `scp` atau `rsync` sebagai penyelesaian sementara.

#### `sftp.list(sessionId, path)`

Senaraikan fail dalam direktori jarak jauh.
```javascript
...
Muat naik fail dari mesin tempatan ke pelayan jarak jauh.
```javascript
...
Muat turun fail dari pelayan jarak jauh ke mesin tempatan.
```javascript
...
Tambah butang ke bar sisi WIA SOOM.

| Pilihan | Jenis | Diperlukan | Penerangan |
|--------|------|----------|-------------|
| `id` | `string` | Tidak | ID unik (secara lalai kepada nama plugin) |
| `icon` | `string` | Ya | Nama ikon Lucide (contohnya, `'server'`, `'shield'`, `'database'`) |
| `label` | `string` | Ya | Teks butang yang ditunjukkan di bar sisi |
...
Buka tetingkap popup dengan kandungan HTML khusus. Inilah cara anda membina UI yang kaya.
...
| `message` | `string` | Teks yang ditunjukkan |
```javascript
...
| `text` | `string` | Teks yang dipaparkan |
```javascript
...
Tetapan plugin disimpan secara kekal dalam `~/.wia-soom/plugins/{your-plugin}/.plugin-settings.json`.
...
> **Status: Akan Datang** — API AI telah ditakrifkan tetapi belum disambungkan ke Soomy. Kini mengembalikan `{ response: 'AI not yet connected' }`. Integrasi AI penuh dirancang untuk keluaran akan datang.
...
| `description` | Keterangan satu baris | `"Monitors nginx access logs in real-time"` |
...
Peraturan ini adalah **wajib**. Plugin yang melanggar akan ditolak.
...
Sentiasa bungkus operasi berisiko dalam try/catch:
```javascript
exports.activate = function(context) {
  try {
    // Your plugin logic
...
Memantau output terminal dan menghantar notifikasi apabila pola tertentu dikesan.
```javascript
'use strict';
...
Gunakan ini dalam `package.json` `keywords` atau ketika mengirim ke registry:
...
| `space` | Alat ruang angkasa dan astronomi |
