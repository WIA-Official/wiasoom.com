...
- Beste Praktiken und Sicherheitsregeln
...
5. Committe mit einer klaren Nachricht:
   ```
   feat: dark mode toggle zu den Einstellungen hinzufügen
   ```
...
- [ ] Keine fest codierten Strings (verwende i18n-Schlüssel)
...
- Die Übersetzung erfolgt über `scripts/translate-patch.js` (GPT-4o-mini API)
...
Viele unserer 254 Sprachen wurden maschinell übersetzt. Überprüfungen durch Muttersprachler sind unglaublich wertvoll!
...
```bash
```
> Hinweis: Der Standard-Heap von 2 GB reicht aufgrund der 254 Sprachdateien + Monaco-Editor-Bundle (~38 MB Renderer) nicht aus.
...
Egal, ob du einen Tippfehler korrigierst, einen Text übersetzt, ein Plugin erstellst oder ein wichtiges Feature hinzufügst — **du bist Teil dieser Geschichte.**
