...
<h1 align="center">對 WIA SOOM 的貢獻</h1>
...
- **要尊重。** 以尊嚴對待每個人。
- **要建設性。** 提供有幫助的反饋，而不是破壞性的批評。
- **要包容。** 我們支持 254 種語言，歡迎來自世界各國的貢獻者。
- **禁止騷擾。** 對任何形式的歧視零容忍。
...
WIA SOOM 擁有強大的插件系統——你可以在 5 分鐘內構建自己的插件。
...
我們使用標準的 ISO 639-1 代碼（例如， `ko`、`en`、`ja`、`ar`、`hi`），並在需要時使用區域變體（例如， `zh-CN`、`pt-BR`）。
...
```bash
```
> 注意：由於 254 種語言文件 + Monaco 編輯器包（約 38MB 渲染器），默認的 2GB 堆內存不夠用。
...
<p align="center"><em>由 SmileStory Inc. 和全球的貢獻者們用 ❤️ 建造。</em></p>