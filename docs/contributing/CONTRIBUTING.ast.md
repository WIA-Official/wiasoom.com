...
- **No acoso.** Tolerancia cero para la discriminación de cualquier tipo.
...
5. Haz un commit con un mensaje claro:
   ```
   feat: añadir interruptor de modo oscuro a configuraciones
   ```
...
#### Opción 2: Añadir claves faltantes
```bash
...
```bash
```
> Nota: El montón predeterminado de 2GB no es suficiente debido a los 254 archivos de idioma + paquete del editor Monaco (~38MB renderizador).
...
Cada contribución fa que WIA SOOM sea meyor pa los desarrolladores de tola parte del mundu.

Ya seya que corrijas un error tipográfico, traduzas una cadena, construyas un plugin, o añadas una característica importante — **tú formes parte d'esta historia.**

---

<p align="center"><em>Construyíu con ❤️ por SmileStory Inc. y contribuyentes de tolu mundu.</em></p>