...
我们致力于为每个人提供一个友好和包容的体验。

- **尊重他人。** 以尊严对待每一个人。
- **建设性。** 提供有帮助的反馈，而不是破坏性的批评。
- **包容性。** 我们支持254种语言，欢迎来自地球上每个国家的贡献者。
- **无骚扰。** 对任何��式的歧视零容忍。
...
我们使用标准的ISO 639-1代码（例如，`ko`，`en`，`ja`，`ar`，`hi`），并在需要时使用区域变体（例如，`zh-CN`，`pt-BR`）。
...
每一个贡献都让 WIA SOOM 对全球开发者变得更好。

无论您是修复一个错别字、翻译一个字符串、构建一个插件，还是添加一个重要功能 — **您都是这个故事的一部分。**

---

<p align="center"><em>由 SmileStory Inc. 和全球贡献者用 ❤️ 构建。</em></p>
```bash
