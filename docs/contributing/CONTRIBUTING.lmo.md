...
<p align="center"><strong>Ci piacerìa riceve i tò contributi!</strong></p>
<p align="center">Che sia una correzione de bug, una nuova funzionalità, un plugin o una traduzione — ogni contributo l'è important.</p>
...
- [Contributi di Traduzione (254 Lingue)](#-contributi-di-traduzione-254-lingue)
...
- **Sii costruttivo.** Offri feedback utile, non critiche distruttive.
- **Sii inclusivo.** Supportiamo 254 lingue e accogliamo contributori da ogni paese del mondo.
...
2. Aggiungi il tuo plugin a `plugins/{nome-del-tuo-plugin}/`
...
5. Fai un commit con un messaggio chiaro:
   ```
   feat: aggiungi il toggle della modalità scura nelle impostazioni
   ```
6. Push e apri una PR contro `main`
...
WIA SOOM supporta **254 lingue** — dall'Amarico allo Zulu, inclusi Braille e lingue RTL.
...
- La traduzione è effettuata tramite `scripts/translate-patch.js` (API GPT-4o-mini)
...
```
#### Opzione 3: Rivedi traduzioni automatiche

Molte delle nostre 254 lingue sono state tradotte automaticamente. Le revisioni di madrelingua sono incredibilmente preziose!
...
Usiamo codici standard ISO 639-1 (es., `ko`, `en`, `ja`, `ar`, `hi`) con varianti regionali dove necessario (es., `zh-CN`, `pt-BR`).
...
Che tu stia correggendo un errore di battitura, traducendo una stringa, costruendo un plugin, o aggiungendo una funzionalità importante — **sei parte di questa storia.**
