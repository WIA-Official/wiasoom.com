...
- **لا للتحرش.** لا تسامح مع التمييز من أي نوع.
...
4. أضف:
   ...
   - لقطات شاشة أو مخرجات طرفية إذا كان ذلك ممكنًا
...
5. قم بالتزام مع رسالة واضحة:
   ```
   feat: إضافة زر تبديل الوضع المظلم إلى الإعدادات
   ```
...
- [ ] الاختبارات الحالية لا تزال تمر

---

## 🌐 مساهمات الترجمة (254 لغة)

تدعم WIA SOOM **254 لغة** — من الأمهرية إلى الزولو، بما في ذلك لغة برايل واللغات من اليمين إلى اليسار.

### كيفية عمل الترجمة

- ملف اللغة الأساسي: `src/renderer/src/i18n/en.json`
- جميع ملفات اللغات الـ 254 في نفس الدليل
- تتم الترجمة عبر `scripts/translate-patch.js` (API GPT-4o-mini)

### كيفية المساهمة في الترجمات

#### الخيار 1: إصلاح ترجمة معينة
...
نستخدم رموز ISO 639-1 القياسية (مثل `ko`, `en`, `ja`, `ar`, `hi`) مع متغيرات إقليمية عند الحاجة (مثل `zh-CN`, `pt-BR`).
...
### البناء
§§§CHUNK_SEPARATOR§§§
> ملاحظة: الذاكرة الافتراضية 2GB غير كافية بسبب ملفات اللغات الـ 254 + حزمة محرر Monaco (~38MB renderer).
...
<p align="center"><em>مبني مع ❤️ بواسطة SmileStory Inc. ومساهمين من جميع أنحاء العالم.</em></p>
```bash
