...
<p align="center"><strong>我们欢迎您的贡献！</strong></p>
...
WIA SOOM 拥有强大的插件系统——您可以在 5 分钟内构建自己的插件。
...
我们 254 种语言中的许多是机器翻译的。母语者的审查非常宝贵！
...
我们使用标准的 ISO 639-1 代码（例如，`ko`、`en`、`ja`、`ar`、`hi`），在需要时使用区域变体（例如，`zh-CN`、`pt-BR`）。
...
### 构建
§§§CHUNK_SEPARATOR§§§
> 注意：由于 254 种语言文件 + Monaco 编辑器包（约 38MB 渲染器），默认的 2GB 堆内存不足。
...
无论您是修复一个错别字、翻译一段字符串、构建一个插件，还是添加一个重要功能 — **您都是这个故事的一部分。**

---

<p align="center"><em>由 SmileStory Inc. 和全球贡献者们 ❤️ 打造。</em></p>
```bash
