...
- **Seja inclusivo.** Apoiamo-nos em 254 idiomas e damos boas-vindas a contribuintes de todos os países do mundo.
...
4. Após a revisão, seu plugin aparecerá na Loja de Plugins para todos os usuários!
...
- A tradução é feita através de `scripts/translate-patch.js` (API GPT-4o-mini)
...
Muitos dos nossos 254 idiomas foram traduzidos por máquina. Revisões de falantes nativos são extremamente valiosas!
...
Usamos códigos padrão ISO 639-1 (por exemplo, `ko`, `en`, `ja`, `ar`, `hi`) com variantes regionais quando necessário (por exemplo, `zh-CN`, `pt-BR`).
...
```bash
```
> Nota: O heap padrão de 2GB não é suficiente devido aos 254 arquivos de idioma + pacote do editor Monaco (~38MB de renderização).
